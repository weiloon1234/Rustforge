...
impl DepositDataTableHooks for DepositDataTableAppHooks {
    ...
    fn authorize(&self, input: &DataTableInput, ctx: &DataTableContext) -> anyhow::Result<bool> {
        ...
        let base_authorized = has_required_permissions(
            &actor.permissions,
            &[Permission::DepositRead.as_str(), Permission::DepositManage.as_str()],
            PermissionMode::Any,
        );
...
pub fn app_deposit_datatable(db: sqlx::PgPool) -> AppDepositDataTable {
    DepositDataTable::new(db).with_hooks(DepositDataTableAppHooks::default())
}

pub fn register_scoped(registry: &mut DataTableRegistry, db: sqlx::PgPool) {
    registry.register_as(SCOPED_KEY, app_deposit_datatable(db));
}
