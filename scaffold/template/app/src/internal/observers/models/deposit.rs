...
pub async fn created(_event: &ModelEvent, _row: &DepositRecord) -> anyhow::Result<()> { Ok(()) }

pub async fn updating(
    _event: &ModelEvent,
    _old_rows: &[DepositRecord],
...
pub async fn deleted(_event: &ModelEvent, _row: &DepositRecord) -> anyhow::Result<()> { Ok(()) }
