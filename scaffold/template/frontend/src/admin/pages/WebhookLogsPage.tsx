...
export default function WebhookLogsPage() {
  ...
  return (
    <DataTable<WebhookLogDatatableRow>
      ...
      api={api}
      perPage={30}
      hiddenColumns={["id", "request_headers", "request_body", "response_body"]}
      ...
      header={() => (
        <div>
          <h1 className="text-2xl font-bold text-foreground">{t("Webhook Logs")}</h1>
          <p className="mt-1 text-sm text-muted">
            {t("Inspect inbound webhook requests and responses")}
          </p>
        </div>
      )}
    />
