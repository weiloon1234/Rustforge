...
function canManageCredits(account: AdminMeOutput | null): boolean {
  if (!account) return false;
  return useAuthStore.hasPermission(PERMISSION.USER_CREDIT, account);
}
  ...
  const { submit, busy, form } = useAutoForm(api, {
    ...
    fields: [
      ...
      },
      {
        ...
        label: t("Credit Type"),
        required: true,
        options: CREDIT_TYPES.map((value) => ({
          value,
          label: t(CREDIT_TYPE_I18N[value] ?? value),
        })),
      },
      {
        name: "amount",
