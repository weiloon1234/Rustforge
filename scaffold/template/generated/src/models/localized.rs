        ...
        if !__old_rows_json.is_empty() && res.rows_affected() > 0 {
            if let Some(observer) = try_get_observer() {
                for (record_id, old_data) in &__old_rows_json {
                    let event = ModelEvent { table: "localized", record_id: *record_id };
...
pub async fn save(self) -> Result<LocalizedView> {
        ...
        match db_conn {
            DbConn::Pool(pool) => {
                ...
                tx.commit().await?;
                if let Some(observer) = try_get_observer() {
                    let event = ModelEvent { table: "localized", record_id: view.id };
        ...
        if !__old_rows_json.is_empty() && res.rows_affected() > 0 {
            if let Some(observer) = try_get_observer() {
                for (record_id, old_data) in &__old_rows_json {
                    ...
                    if let Ok(Some(new_row)) = db.fetch_optional(sqlx::query_as::<_, LocalizedRow>(&fetch_sql).bind(record_id)).await {
