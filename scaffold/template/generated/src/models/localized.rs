...
use crate::generated::models::common::{renumber_placeholders, Page};
use anyhow::Result;
use core_datatable::{
    AutoDataTable, BoxFuture, DataTableColumnDescriptor, DataTableContext, DataTableInput,
    DataTableRelationColumnDescriptor, GeneratedTableAdapter, ParsedFilter, SortDirection,
};
use core_db::common::collection::TypedCollectionExt;
use core_db::common::pagination::resolve_per_page;
use core_db::common::sql::{
    bind, bind_query, bind_scalar, BindValue, DbConn, Op, OrderDir, RawClause, RawGroupExpr,
    RawJoinKind, RawJoinSpec, RawOrderExpr, RawSelectExpr,
};
...
impl LocalizedView {
    pub fn update<'db>(&self, db: impl Into<DbConn<'db>>) -> LocalizedUpdate<'db> {
        Localized::new(db.into(), None)
            .update()
            .where_id(Op::Eq, self.id.clone())
    }
    pub fn update_with<'db>(&self, model: &Localized<'db>) -> LocalizedUpdate<'db> {
        model.update().where_id(Op::Eq, self.id.clone())
    }
    pub fn to_json(&self) -> LocalizedJson {
        LocalizedJson {
            id: self.id.clone(),
...
pub trait LocalizedViewsExt {
    fn ids(&self) -> Vec<i64>;
    fn pluck<R>(&self, f: impl Fn(&LocalizedView) -> R) -> Vec<R>;
    fn key_by<K>(
        &self,
        f: impl Fn(&LocalizedView) -> K,
    ) -> std::collections::HashMap<K, LocalizedView>
    where
        K: Eq + std::hash::Hash;
    fn group_by<K>(
        &self,
        f: impl Fn(&LocalizedView) -> K,
    ) -> std::collections::HashMap<K, Vec<LocalizedView>>
    where
        K: Eq + std::hash::Hash;
}

impl LocalizedViewsExt for Vec<LocalizedView> {
    fn ids(&self) -> Vec<i64> {
        self.as_slice().pluck_typed(|v| v.id.clone())
    }
    fn pluck<R>(&self, f: impl Fn(&LocalizedView) -> R) -> Vec<R> {
...
impl LocalizedCol {
    pub const fn all() -> &'static [LocalizedCol] {
        &[
            LocalizedCol::Id,
            LocalizedCol::OwnerType,
            LocalizedCol::OwnerId,
            LocalizedCol::Field,
            LocalizedCol::Locale,
            LocalizedCol::Value,
        ]
    ...
    pub const fn as_sql(self) -> &'static str {
        match self {
            LocalizedCol::Id => "id",
            LocalizedCol::OwnerType => "owner_type",
            LocalizedCol::OwnerId => "owner_id",
...
pub struct Localized<'db> {
    db: DbConn<'db>,
    base_url: Option<String>,
}

impl<'db> Localized<'db> {
    pub const TABLE: &'static str = "localized";
    pub const PK: &'static str = "id";
    pub fn new(db: impl Into<DbConn<'db>>, base_url: Option<String>) -> Self {
        Self {
            db: db.into(),
            base_url,
        }
    }
    pub fn query(&self) -> LocalizedQuery<'db> {
        LocalizedQuery::new(self.db.clone(), self.base_url.clone())
    }
    pub fn insert(&self) -> LocalizedInsert<'db> {
        LocalizedInsert::new(self.db.clone(), self.base_url.clone())
    }
    pub fn update(&self) -> LocalizedUpdate<'db> {
        LocalizedUpdate::new(self.db.clone(), self.base_url.clone())
    }
    pub async fn find(&self, id: i64) -> Result<Option<LocalizedView>> {
...
impl<'db> LocalizedQuery<'db> {
    pub fn new(db: DbConn<'db>, base_url: Option<String>) -> Self {
        Self {
            db,
            base_url,
            select_sql: Some("id, owner_type, owner_id, field, locale, value".to_string()),
            from_sql: None,
            count_sql: None,
            distinct: false,
            distinct_on: None,
    ...
    pub fn unsafe_sql(self) -> LocalizedUnsafeQuery<'db> {
        LocalizedUnsafeQuery::new(self)
    }
    pub fn where_id(mut self, op: Op, val: i64) -> Self {
        let idx = self.binds.len() + 1;
        self.where_sql.push(format!(
            "{} {} ${}",
            LocalizedCol::Id.as_sql(),
            op.as_sql(),
            idx
        ));
    ...
    pub fn where_key(self, id: i64) -> Self {
        self.where_id(Op::Eq, id)
    }
    pub fn where_key_in<T: Clone + Into<BindValue>>(self, vals: &[T]) -> Self {
        self.where_in(LocalizedCol::Id, vals)
    }
    pub fn where_col<T: Into<BindValue>>(mut self, col: LocalizedCol, op: Op, val: T) -> Self {
        let idx = self.binds.len() + 1;
        self.where_sql
            .push(format!("{} {} ${}", col.as_sql(), op.as_sql(), idx));
        self.binds.push(val.into());
        self
    }
    fn where_raw<T: Into<BindValue>>(
        mut self,
    ...
    pub fn where_in<T: Clone + Into<BindValue>>(mut self, col: LocalizedCol, vals: &[T]) -> Self {
        if vals.is_empty() {
    ...
    pub fn where_not_in<T: Clone + Into<BindValue>>(
        mut self,
        col: LocalizedCol,
        vals: &[T],
    ) -> Self {
    ...
    pub fn where_between<T: Into<BindValue>>(mut self, col: LocalizedCol, low: T, high: T) -> Self {
        let idx1 = self.binds.len() + 1;
        let idx2 = idx1 + 1;
        self.where_sql
            .push(format!("{} BETWEEN ${} AND ${}", col.as_sql(), idx1, idx2));
        self.binds.push(low.into());
        self.binds.push(high.into());
        self
    }
    pub fn where_null(mut self, col: LocalizedCol) -> Self {
        self.where_sql.push(format!("{} IS NULL", col.as_sql()));
        self
    }
    pub fn where_not_null(mut self, col: LocalizedCol) -> Self {
        self.where_sql.push(format!("{} IS NOT NULL", col.as_sql()));
        self
    }
    pub fn or_where_col<T: Into<BindValue>>(mut self, col: LocalizedCol, op: Op, val: T) -> Self {
        let idx = self.binds.len() + 1;
    ...
    fn or_where_raw<T: Into<BindValue>>(
        mut self,
        clause: impl Into<String>,
        binds: impl IntoIterator<Item = T>,
    ) -> Self {
    ...
    pub fn select_cols(mut self, cols: &[LocalizedCol]) -> Self {
        if cols.is_empty() {
            ...
            let mut list: Vec<String> = "id, owner_type, owner_id, field, locale, value"
                .split(',')
    ...
    fn select_raw(mut self, sql: impl Into<String>) -> Self {
        ...
        if s.is_empty() {
            ...
            self.select_sql = Some(format!(
                "id, owner_type, owner_id, field, locale, value, {}",
                s
            ));
        }
        self
    ...
    fn add_select_raw(mut self, sql: impl Into<String>) -> Self {
        ...
        let mut base = self
            .select_sql
            .take()
            .unwrap_or_else(|| "id, owner_type, owner_id, field, locale, value".to_string());
        if !base.is_empty() {
            base.push_str(", ");
        }
        base.push_str(&s);
    ...
    fn inner_join_raw<T: Into<BindValue>>(
        mut self,
    ...
    fn left_join_raw<T: Into<BindValue>>(
        mut self,
        table: impl Into<String>,
        on_clause: impl Into<String>,
        binds: impl IntoIterator<Item = T>,
    ...
    fn right_join_raw<T: Into<BindValue>>(
        mut self,
        table: impl Into<String>,
        on_clause: impl Into<String>,
        binds: impl IntoIterator<Item = T>,
    ...
    fn full_join_raw<T: Into<BindValue>>(
        mut self,
        table: impl Into<String>,
        on_clause: impl Into<String>,
        binds: impl IntoIterator<Item = T>,
    ...
    pub fn order_by(mut self, col: LocalizedCol, dir: OrderDir) -> Self {
        self.order_sql
            .push(format!("{} {}", col.as_sql(), dir.as_sql()));
        self
    }
    pub fn order_by_nulls_first(mut self, col: LocalizedCol, dir: OrderDir) -> Self {
        self.order_sql
            .push(format!("{} {} NULLS FIRST", col.as_sql(), dir.as_sql()));
        self
    }
    pub fn order_by_nulls_last(mut self, col: LocalizedCol, dir: OrderDir) -> Self {
        self.order_sql
            .push(format!("{} {} NULLS LAST", col.as_sql(), dir.as_sql()));
        self
    }
    ...
    pub fn distinct_on(mut self, cols: &[LocalizedCol]) -> Self {
        if cols.is_empty() {
            return self;
        }
    ...
    pub fn select(mut self, cols: &[LocalizedCol]) -> Self {
    ...
    fn where_exists<T: Into<BindValue>>(
        mut self,
        clause: impl Into<String>,
        binds: impl IntoIterator<Item = T>,
    ) -> Self {
    ...
    pub fn group_by(mut self, cols: &[LocalizedCol]) -> Self {
        for c in cols {
            self.group_by_sql.push(c.as_sql().to_string());
        }
        self
    }
    pub fn having_raw<T: Into<BindValue>>(
        mut self,
        clause: impl Into<String>,
        binds: impl IntoIterator<Item = T>,
    ) -> Self {
        ...
        T: for<'r> sqlx::FromRow<'r, sqlx::postgres::PgRow> + Send + Unpin + 'static,
    {
        ...
        let select_clause = match (distinct, distinct_on.as_ref()) {
            ...
            (_, Some(on)) => format!(
                "DISTINCT ON ({}) {}",
                on,
                select_sql.unwrap_or_else(|| "*".to_string())
            ),
        };
    ...
    pub async fn get(self) -> Result<Vec<LocalizedView>> {
        ...
        for r in rows {
            out_vec.push(hydrate_view(
                r,
                &LocalizedMap::default(),
                base_url.as_deref(),
            ));
        }
        Ok(out_vec)
    ...
    pub async fn first(self) -> Result<Option<LocalizedView>> {
        let mut v = self.limit(1).get().await?;
        Ok(v.pop())
    }

    pub async fn first_or_fail(self) -> Result<LocalizedView> {
        self.first()
            .await?
            .ok_or_else(|| anyhow::anyhow!("localized: record not found"))
    }

    pub async fn find(self, id: i64) -> Result<Option<LocalizedView>> {
        self.where_id(Op::Eq, id).first().await
    }
    pub async fn find_or_fail(self, id: i64) -> Result<LocalizedView> {
        self.find(id)
            .await?
            .ok_or_else(|| anyhow::anyhow!("localized: record not found"))
    }
    pub async fn first_or_create(
        self,
        create: impl FnOnce(LocalizedInsert<'db>) -> LocalizedInsert<'db>,
    ) -> Result<LocalizedView> {
        let db = self.db.clone();
        let base_url = self.base_url.clone();
        if let Some(existing) = self.first().await? {
            return Ok(existing);
        }
        let insert_builder = create(LocalizedInsert::new(db, base_url));
        insert_builder.save().await
    }

    pub async fn update_or_create(
        self,
        on_update: impl FnOnce(LocalizedUpdate<'db>) -> LocalizedUpdate<'db>,
        on_create: impl FnOnce(LocalizedInsert<'db>) -> LocalizedInsert<'db>,
    ) -> Result<LocalizedView> {
        let db = self.db.clone();
        let base_url = self.base_url.clone();
        ...
        if let Some(existing) = self.first().await? {
            let mut update_builder = LocalizedUpdate::new(db.clone(), base_url.clone());
            ...
            return Localized::new(db, base_url.clone())
                .query()
                .find(existing.id)
                .await
                .map(|r| r.unwrap());
        }
        let insert_builder = on_create(LocalizedInsert::new(db, base_url));
        insert_builder.save().await
    }

    pub async fn increment(self, col: LocalizedCol, amount: i64) -> Result<u64> {
        let db = self.db.clone();
    ...
    pub async fn decrement(self, col: LocalizedCol, amount: i64) -> Result<u64> {
    ...
    pub async fn pluck_ids(self) -> Result<Vec<i64>> {
        ...
        let order_clause = if order_sql.is_empty() {
            String::new()
        } else {
            format!(" ORDER BY {}", order_sql.join(", "))
        };
        let limit_clause = limit.map(|n| format!(" LIMIT {}", n)).unwrap_or_default();
        let offset_clause = offset.map(|n| format!(" OFFSET {}", n)).unwrap_or_default();
        let sql = format!(
            "SELECT id {}{}{}{}{}",
            from_clause, where_clause, order_clause, limit_clause, offset_clause
        );
        let mut q = sqlx::query_scalar::<_, i64>(&sql);
        for b in binds {
            q = bind_scalar(q, b);
    ...
    where
        F: FnMut(Vec<LocalizedView>) -> Fut,
        Fut: std::future::Future<Output = Result<bool>>,
    {
        let mut page = 0i64;
        let db = self.db.clone();
        loop {
            let mut query = LocalizedQuery::new(db.clone(), self.base_url.clone());
    ...
    pub async fn sole(self) -> Result<LocalizedView> {
    ...
    pub async fn pluck_pair<K, V>(
        self,
        extract: impl Fn(&LocalizedView) -> (K, V),
    ...
    pub async fn sum(self, col: LocalizedCol) -> Result<Option<f64>> {
        let Self {
            db,
            from_sql,
            join_sql,
    ...
    pub async fn avg(self, col: LocalizedCol) -> Result<Option<f64>> {
        let Self {
            db,
            from_sql,
            join_sql,
    ...
    pub async fn min_val(self, col: LocalizedCol) -> Result<Option<i64>> {
        let Self {
            db,
            from_sql,
            join_sql,
    ...
    pub async fn max_val(self, col: LocalizedCol) -> Result<Option<i64>> {
        let Self {
            db,
            from_sql,
            join_sql,
    ...
    pub async fn paginate(self, page: i64, per_page: i64) -> Result<Page<LocalizedView>> {
        ...
        for b in binds.iter().cloned() {
            count_q = bind_scalar(count_q, b);
        }
        for b in join_binds.iter().cloned() {
            count_q = bind_scalar(count_q, b);
        }
...
impl<'db> LocalizedUnsafeQuery<'db> {
    fn new(inner: LocalizedQuery<'db>) -> Self {
        Self { inner }
    }
    pub fn where_raw(mut self, clause: RawClause) -> Self {
        let (sql, binds) = clause.into_parts();
        self.inner = self.inner.where_raw(sql, binds);
        self
    }
    pub fn or_where_raw(mut self, clause: RawClause) -> Self {
        let (sql, binds) = clause.into_parts();
        self.inner = self.inner.or_where_raw(sql, binds);
        self
    }
    pub fn join_raw(mut self, spec: RawJoinSpec) -> Self {
        let (kind, table, on, binds) = spec.into_parts();
        self.inner = match kind {
            RawJoinKind::Inner => self.inner.inner_join_raw(table, on, binds),
            RawJoinKind::Left => self.inner.left_join_raw(table, on, binds),
            RawJoinKind::Right => self.inner.right_join_raw(table, on, binds),
            RawJoinKind::Full => self.inner.full_join_raw(table, on, binds),
        };
        self
    }
    pub fn select_raw(mut self, expr: RawSelectExpr) -> Self {
        self.inner = self.inner.select_raw(expr.into_inner());
        self
    }
    pub fn add_select_raw(mut self, expr: RawSelectExpr) -> Self {
        self.inner = self.inner.add_select_raw(expr.into_inner());
        self
    }
    pub fn select_subquery(mut self, alias: impl Into<String>, sql: RawSelectExpr) -> Self {
        let alias = alias.into();
        let raw = sql.into_inner();
        self.inner = self.inner.select_subquery(&alias, &raw);
        self
    }
    pub fn from_raw(mut self, expr: RawSelectExpr) -> Self {
        let raw = expr.into_inner();
        self.inner = self.inner.from_raw(&raw);
        self
    }
    pub fn count_sql(mut self, expr: RawSelectExpr) -> Self {
        let raw = expr.into_inner();
        self.inner = self.inner.count_sql(&raw);
        self
    }
    pub fn where_exists(mut self, clause: RawClause) -> Self {
        let (sql, binds) = clause.into_parts();
        self.inner = self.inner.where_exists(sql, binds);
        self
    }
    pub fn order_by_raw(mut self, expr: RawOrderExpr) -> Self {
        self.inner = self.inner.order_by_raw(expr.into_inner());
        self
    }
    pub fn group_by_raw(mut self, expr: RawGroupExpr) -> Self {
        self.inner = self.inner.group_by_raw(expr.into_inner());
        self
    }
    pub fn done(self) -> LocalizedQuery<'db> {
        self.inner
    }
}
...
pub struct LocalizedInsert<'db> {
    db: DbConn<'db>,
    base_url: Option<String>,
    cols: Vec<LocalizedCol>,
    binds: Vec<BindValue>,
    conflict_action: Option<&'static str>,
    conflict_cols: Vec<LocalizedCol>,
}

impl<'db> LocalizedInsert<'db> {
    pub fn new(db: DbConn<'db>, base_url: Option<String>) -> Self {
        Self {
            db,
            base_url,
    ...
    pub fn set_owner_type(mut self, val: String) -> Self {
        self.cols.push(LocalizedCol::OwnerType);
        self.binds.push(val.into());
        self
    }
    pub fn set_owner_id(mut self, val: i64) -> Self {
        self.cols.push(LocalizedCol::OwnerId);
        self.binds.push(val.into());
        self
    }
    pub fn set_field(mut self, val: String) -> Self {
        self.cols.push(LocalizedCol::Field);
        self.binds.push(val.into());
        self
    }
    pub fn set_locale(mut self, val: String) -> Self {
        self.cols.push(LocalizedCol::Locale);
        self.binds.push(val.into());
        self
    }
    pub fn set_value(mut self, val: String) -> Self {
        self.cols.push(LocalizedCol::Value);
        self.binds.push(val.into());
        self
    }
    ...
    pub fn on_conflict_update(mut self, conflict_cols: &[LocalizedCol]) -> Self {
    ...
    async fn save_with_db<'tx>(self, db: DbConn<'tx>) -> Result<LocalizedView> {
        ...
        if let Some(action) = self.conflict_action {
            if !self.conflict_cols.is_empty() {
                let conflict_col_sql: Vec<&'static str> =
                    self.conflict_cols.iter().map(|c| c.as_sql()).collect();
                sql.push_str(&format!(
                    " ON CONFLICT ({}) {}",
                    conflict_col_sql.join(", "),
                    action
                ));
                if action == "DO UPDATE" {
                    let set_clauses: Vec<String> = col_sql
                        .iter()
...
pub struct LocalizedUpdate<'db> {
    db: DbConn<'db>,
    base_url: Option<String>,
    sets: Vec<(LocalizedCol, BindValue)>,
    where_sql: Vec<String>,
    binds: Vec<BindValue>,
}

impl<'db> LocalizedUpdate<'db> {
    pub fn new(db: DbConn<'db>, base_url: Option<String>) -> Self {
        Self {
            db,
            base_url,
    ...
    pub fn unsafe_sql(self) -> LocalizedUnsafeUpdate<'db> {
        LocalizedUnsafeUpdate::new(self)
    }
    pub fn set_id(mut self, val: i64) -> Self {
    ...
    pub fn set_owner_type(mut self, val: String) -> Self {
        self.sets.push((LocalizedCol::OwnerType, val.into()));
        self
    }
    pub fn set_owner_id(mut self, val: i64) -> Self {
        self.sets.push((LocalizedCol::OwnerId, val.into()));
        self
    }
    pub fn set_field(mut self, val: String) -> Self {
        self.sets.push((LocalizedCol::Field, val.into()));
        self
    }
    pub fn set_locale(mut self, val: String) -> Self {
        self.sets.push((LocalizedCol::Locale, val.into()));
        self
    }
    pub fn set_value(mut self, val: String) -> Self {
        self.sets.push((LocalizedCol::Value, val.into()));
        self
    }
    pub fn where_id(mut self, op: Op, val: i64) -> Self {
        let idx = self.binds.len() + 1;
    ...
    async fn save_with_db<'tx>(self, db: DbConn<'tx>) -> Result<u64> {
        ...
        let select_sql = format!(
            "SELECT id FROM localized WHERE {}",
            self.where_sql.join(" AND ")
        );
        let mut select_q = sqlx::query_scalar::<_, i64>(&select_sql);
        for b in &self.binds {
            select_q = bind_scalar(select_q, b.clone());
        }
...
impl<'db> LocalizedUnsafeUpdate<'db> {
    fn new(inner: LocalizedUpdate<'db>) -> Self {
        Self { inner }
    }
    pub fn where_raw(mut self, clause: RawClause) -> Self {
        let (sql, binds) = clause.into_parts();
    ...
    pub fn done(self) -> LocalizedUpdate<'db> {
        self.inner
    }
}
...
impl LocalizedTableAdapter {
    fn parse_col(name: &str) -> Option<LocalizedCol> {
        match name {
            "id" => Some(LocalizedCol::Id),
            "owner_type" => Some(LocalizedCol::OwnerType),
            "owner_id" => Some(LocalizedCol::OwnerId),
    ...
    fn parse_like_col(name: &str) -> Option<LocalizedCol> {
        match name {
            "owner_type" => Some(LocalizedCol::OwnerType),
            "field" => Some(LocalizedCol::Field),
            "locale" => Some(LocalizedCol::Locale),
    ...
    fn parse_bind(raw: &str) -> BindValue {
        ...
        if let Ok(v) = uuid::Uuid::parse_str(trimmed) {
            return v.into();
        }
        if let Some(v) = Self::parse_datetime(trimmed, false) {
            return v.into();
        }
...
impl GeneratedTableAdapter for LocalizedTableAdapter {
    ...
    fn apply_auto_filter<'db>(
        &self,
        query: LocalizedQuery<'db>,
        filter: &ParsedFilter,
        value: &str,
    ) -> anyhow::Result<Option<LocalizedQuery<'db>>>
    where
        Self: 'db,
    {
        ...
        match filter {
            ParsedFilter::Eq { column } => {
                let Some(col) = Self::parse_col(column.as_str()) else {
                    return Ok(None);
                };
                let Some(bind) = Self::parse_bind_for_col(column.as_str(), trimmed) else {
                    return Ok(None);
                };
                Ok(Some(query.where_col(col, Op::Eq, bind)))
            }
            ParsedFilter::Like { column } => {
                let Some(col) = Self::parse_like_col(column.as_str()) else {
                    return Ok(None);
                };
            ...
            ParsedFilter::DateFrom { column } => {
                ...
                let Some(ts) = Self::parse_datetime(trimmed, false) else {
                    return Ok(None);
                };
            ...
            ParsedFilter::DateTo { column } => {
                ...
                let Some(ts) = Self::parse_datetime(trimmed, true) else {
                    return Ok(None);
                };
                Ok(Some(query.where_col(col, Op::Le, ts)))
            }
            ParsedFilter::LocaleEq { column } => Ok(None),
            ParsedFilter::LocaleLike { column } => Ok(None),
                ...
                let next = query.where_group(|group| {
                    ...
                    for column in columns {
                        if let Some(col) = Self::parse_like_col(column.as_str()) {
                            if applied {
                                q = q.or_where_col(col, Op::Like, pattern.clone());
                            } else {
                                q = q.where_col(col, Op::Like, pattern.clone());
                                applied = true;
                            }
                        }
                    }
                ...
                let next = query.where_group(|group| {
                    ...
                    for column in columns {
                        if let Some(col) = Self::parse_col(column.as_str()) {
                            if let Some(bind) = Self::parse_bind_for_col(column.as_str(), trimmed) {
                                if applied {
                                    q = q.or_where_col(col, Op::Eq, bind.clone());
                                } else {
                                    q = q.where_col(col, Op::Eq, bind.clone());
                                    applied = true;
                                }
                            }
                        }
            ...
            ParsedFilter::Has { relation, column } => match (relation.as_str(), column.as_str()) {
                _ => Ok(None),
            },
            ...
            ParsedFilter::LocaleHas { relation, column } => {
                let Some(field) =
                    Self::parse_locale_field_for_relation(relation.as_str(), column.as_str())
                else {
                    return Ok(None);
                };
    ...
    fn apply_sort<'db>(
        ...
        column: &str,
        dir: SortDirection,
    ) -> anyhow::Result<LocalizedQuery<'db>>
    where
        Self: 'db,
    {
        let dir = match dir {
        ...
        let next = match column {
            "id" => query.order_by(LocalizedCol::Id, dir),
            "owner_type" => query.order_by(LocalizedCol::OwnerType, dir),
            "owner_id" => query.order_by(LocalizedCol::OwnerId, dir),
    ...
    where
        Self: 'db,
    {
        let Some(col) = Self::parse_col(column) else {
            return Ok(None);
        };
        let Some(bind) = Self::parse_bind_for_col(column, cursor) else {
            return Ok(None);
        };
        let op = match dir {
            SortDirection::Asc => Op::Gt,
            SortDirection::Desc => Op::Lt,
        };
        Ok(Some(query.where_col(col, op, bind)))
    }
    fn cursor_from_row(&self, row: &LocalizedView, column: &str) -> Option<String> {
        match column {
    ...
    fn count<'db>(&self, query: LocalizedQuery<'db>) -> BoxFuture<'db, anyhow::Result<i64>>
    where
        Self: 'db,
    {
    ...
    fn fetch_page<'db>(
        ...
        page: i64,
        per_page: i64,
    ) -> BoxFuture<'db, anyhow::Result<Vec<LocalizedView>>>
    where
        Self: 'db,
    {
...
pub trait LocalizedDataTableHooks: Send + Sync + 'static {
    ...
    fn authorize(&self, _input: &DataTableInput, _ctx: &DataTableContext) -> anyhow::Result<bool> {
        Ok(true)
    }
    fn filter_query<'db>(
        &'db self,
        _query: LocalizedQuery<'db>,
        _filter_key: &str,
        _value: &str,
        _input: &DataTableInput,
        _ctx: &DataTableContext,
    ...
    fn map_row(
        &self,
        _row: &mut LocalizedView,
        _input: &DataTableInput,
        _ctx: &DataTableContext,
    ) -> anyhow::Result<()> {
        Ok(())
    }
    fn default_row_to_record(
        &self,
        row: LocalizedView,
...
pub struct LocalizedDataTable<H = LocalizedDefaultDataTableHooks>
where
    H: LocalizedDataTableHooks,
{
    pub db: sqlx::PgPool,
    pub hooks: H,
...
impl<H: LocalizedDataTableHooks> AutoDataTable for LocalizedDataTable<H> {
    ...
    fn base_query<'db>(
        ...
        self.hooks
            .scope(Localized::new(&self.db, None).query(), input, ctx)
    }
    ...
    fn filter_query<'db>(
        ...
        self.hooks
            .filter_query(query, filter_key, value, input, ctx)
    }
    fn filters<'db>(
        &'db self,
        query: LocalizedQuery<'db>,
        ...
        self.hooks.filters(query, input, ctx)
    }
    fn map_row(
        &self,
        row: &mut LocalizedView,
        input: &DataTableInput,
        ctx: &DataTableContext,
    ) -> anyhow::Result<()> {
        self.hooks.map_row(row, input, ctx)
    }
    fn row_to_record(
        &self,
        row: LocalizedView,
        ...
        self.hooks.row_to_record(row, input, ctx)
    }
    fn summary<'db>(
        &'db self,
        query: LocalizedQuery<'db>,
