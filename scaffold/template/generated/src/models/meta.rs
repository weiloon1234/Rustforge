...
impl MetaView {
    pub fn update<'db>(&self, db: impl Into<DbConn<'db>>) -> MetaUpdate<'db> {
        Meta::new(db.into(), None)
            .update()
            .where_id(Op::Eq, self.id.clone())
    }
    pub fn update_with<'db>(&self, model: &Meta<'db>) -> MetaUpdate<'db> {
        model.update().where_id(Op::Eq, self.id.clone())
    }
    pub fn to_json(&self) -> MetaJson {
        MetaJson {
            id: self.id.clone(),
...
pub trait MetaViewsExt {
    fn ids(&self) -> Vec<i64>;
    fn pluck<R>(&self, f: impl Fn(&MetaView) -> R) -> Vec<R>;
    fn key_by<K>(&self, f: impl Fn(&MetaView) -> K) -> std::collections::HashMap<K, MetaView>
    where
        K: Eq + std::hash::Hash;
    fn group_by<K>(
        &self,
        f: impl Fn(&MetaView) -> K,
    ) -> std::collections::HashMap<K, Vec<MetaView>>
    where
        K: Eq + std::hash::Hash;
}

impl MetaViewsExt for Vec<MetaView> {
    fn ids(&self) -> Vec<i64> {
    ...
    fn group_by<K>(&self, f: impl Fn(&MetaView) -> K) -> std::collections::HashMap<K, Vec<MetaView>>
    where
        K: Eq + std::hash::Hash,
    {
...
impl<'db> Meta<'db> {
    ...
    pub fn query(&self) -> MetaQuery<'db> {
        MetaQuery::new(self.db.clone(), self.base_url.clone())
    }
    pub fn insert(&self) -> MetaInsert<'db> {
        MetaInsert::new(self.db.clone(), self.base_url.clone())
    }
    pub fn update(&self) -> MetaUpdate<'db> {
        MetaUpdate::new(self.db.clone(), self.base_url.clone())
    }
    pub async fn find(&self, id: i64) -> Result<Option<MetaView>> {
...
impl<'db> MetaQuery<'db> {
    pub fn new(db: DbConn<'db>, base_url: Option<String>) -> Self {
        Self {
            db,
            base_url,
            select_sql: Some("id, owner_type, owner_id, field, value".to_string()),
            from_sql: None,
            count_sql: None,
            distinct: false,
            distinct_on: None,
    ...
    pub fn unsafe_sql(self) -> MetaUnsafeQuery<'db> {
        MetaUnsafeQuery::new(self)
    }
    pub fn where_id(mut self, op: Op, val: i64) -> Self {
        let idx = self.binds.len() + 1;
        self.where_sql
            .push(format!("{} {} ${}", MetaCol::Id.as_sql(), op.as_sql(), idx));
    ...
    pub fn where_key_in<T: Clone + Into<BindValue>>(self, vals: &[T]) -> Self {
        self.where_in(MetaCol::Id, vals)
    }
    pub fn where_col<T: Into<BindValue>>(mut self, col: MetaCol, op: Op, val: T) -> Self {
        let idx = self.binds.len() + 1;
        self.where_sql
    ...
    pub fn where_in<T: Clone + Into<BindValue>>(mut self, col: MetaCol, vals: &[T]) -> Self {
        if vals.is_empty() {
    ...
    pub fn where_not_in<T: Clone + Into<BindValue>>(mut self, col: MetaCol, vals: &[T]) -> Self {
        if vals.is_empty() {
            return self;
        }
        let start = self.binds.len() + 1;
    ...
    pub fn where_between<T: Into<BindValue>>(mut self, col: MetaCol, low: T, high: T) -> Self {
    ...
    pub fn where_null(mut self, col: MetaCol) -> Self {
        self.where_sql.push(format!("{} IS NULL", col.as_sql()));
        self
    }
    pub fn where_not_null(mut self, col: MetaCol) -> Self {
        self.where_sql.push(format!("{} IS NOT NULL", col.as_sql()));
        self
    }
    pub fn or_where_col<T: Into<BindValue>>(mut self, col: MetaCol, op: Op, val: T) -> Self {
        let idx = self.binds.len() + 1;
    ...
    pub fn select_cols(mut self, cols: &[MetaCol]) -> Self {
        if cols.is_empty() {
            ...
            let mut list: Vec<String> = "id, owner_type, owner_id, field, value"
                .split(',')
    ...
    fn add_select_raw(mut self, sql: impl Into<String>) -> Self {
        ...
        let mut base = self
            .select_sql
            .take()
            .unwrap_or_else(|| "id, owner_type, owner_id, field, value".to_string());
        if !base.is_empty() {
            base.push_str(", ");
        }
        base.push_str(&s);
    ...
    pub fn order_by(mut self, col: MetaCol, dir: OrderDir) -> Self {
        self.order_sql
            .push(format!("{} {}", col.as_sql(), dir.as_sql()));
        self
    }
    pub fn order_by_nulls_first(mut self, col: MetaCol, dir: OrderDir) -> Self {
        self.order_sql
            .push(format!("{} {} NULLS FIRST", col.as_sql(), dir.as_sql()));
        self
    }
    pub fn order_by_nulls_last(mut self, col: MetaCol, dir: OrderDir) -> Self {
        self.order_sql
    ...
    pub fn distinct_on(mut self, cols: &[MetaCol]) -> Self {
        if cols.is_empty() {
            return self;
        }
    ...
    pub fn select(mut self, cols: &[MetaCol]) -> Self {
    ...
    pub fn group_by(mut self, cols: &[MetaCol]) -> Self {
        for c in cols {
    ...
    pub async fn get(self) -> Result<Vec<MetaView>> {
        let Self {
            db,
            base_url,
            select_sql,
    ...
    pub async fn first(self) -> Result<Option<MetaView>> {
        let mut v = self.limit(1).get().await?;
        Ok(v.pop())
    }

    pub async fn first_or_fail(self) -> Result<MetaView> {
        self.first()
            .await?
            .ok_or_else(|| anyhow::anyhow!("meta: record not found"))
    }

    pub async fn find(self, id: i64) -> Result<Option<MetaView>> {
        self.where_id(Op::Eq, id).first().await
    }
    pub async fn find_or_fail(self, id: i64) -> Result<MetaView> {
        self.find(id)
            .await?
            .ok_or_else(|| anyhow::anyhow!("meta: record not found"))
    }
    pub async fn first_or_create(
        self,
        create: impl FnOnce(MetaInsert<'db>) -> MetaInsert<'db>,
    ) -> Result<MetaView> {
        ...
        let insert_builder = create(MetaInsert::new(db, base_url));
        insert_builder.save().await
    }

    pub async fn update_or_create(
        self,
        on_update: impl FnOnce(MetaUpdate<'db>) -> MetaUpdate<'db>,
        on_create: impl FnOnce(MetaInsert<'db>) -> MetaInsert<'db>,
    ) -> Result<MetaView> {
        let db = self.db.clone();
        let base_url = self.base_url.clone();
        ...
        if let Some(existing) = self.first().await? {
            let mut update_builder = MetaUpdate::new(db.clone(), base_url.clone());
            ...
            return Meta::new(db, base_url.clone())
                .query()
                .find(existing.id)
                .await
                .map(|r| r.unwrap());
        }
        let insert_builder = on_create(MetaInsert::new(db, base_url));
        insert_builder.save().await
    }

    pub async fn increment(self, col: MetaCol, amount: i64) -> Result<u64> {
        let db = self.db.clone();
    ...
    pub async fn decrement(self, col: MetaCol, amount: i64) -> Result<u64> {
    ...
    where
        F: FnMut(Vec<MetaView>) -> Fut,
        Fut: std::future::Future<Output = Result<bool>>,
    {
        let mut page = 0i64;
        let db = self.db.clone();
        loop {
            let mut query = MetaQuery::new(db.clone(), self.base_url.clone());
    ...
    pub async fn sole(self) -> Result<MetaView> {
    ...
    pub async fn sum(self, col: MetaCol) -> Result<Option<f64>> {
        let Self {
            db,
            from_sql,
            join_sql,
    ...
    pub async fn avg(self, col: MetaCol) -> Result<Option<f64>> {
        let Self {
            db,
            from_sql,
            join_sql,
    ...
    pub async fn min_val(self, col: MetaCol) -> Result<Option<i64>> {
        let Self {
            db,
            from_sql,
            join_sql,
    ...
    pub async fn max_val(self, col: MetaCol) -> Result<Option<i64>> {
        let Self {
            db,
            from_sql,
            join_sql,
    ...
    pub async fn paginate(self, page: i64, per_page: i64) -> Result<Page<MetaView>> {
...
#[doc(hidden)]
pub struct MetaUnsafeQuery<'db> {
    inner: MetaQuery<'db>,
}

impl<'db> MetaUnsafeQuery<'db> {
    fn new(inner: MetaQuery<'db>) -> Self {
        Self { inner }
    }
    ...
    pub fn done(self) -> MetaQuery<'db> {
        self.inner
    }
}

pub struct MetaInsert<'db> {
    db: DbConn<'db>,
    base_url: Option<String>,
    cols: Vec<MetaCol>,
    binds: Vec<BindValue>,
    conflict_action: Option<&'static str>,
    conflict_cols: Vec<MetaCol>,
}

impl<'db> MetaInsert<'db> {
    pub fn new(db: DbConn<'db>, base_url: Option<String>) -> Self {
        Self {
            db,
            base_url,
    ...
    pub fn on_conflict_update(mut self, conflict_cols: &[MetaCol]) -> Self {
        self.conflict_action = Some("DO UPDATE");
        self.conflict_cols = conflict_cols.to_vec();
        self
    }
    pub async fn save(self) -> Result<MetaView> {
    ...
    async fn save_with_db<'tx>(self, db: DbConn<'tx>) -> Result<MetaView> {
...
pub struct MetaUpdate<'db> {
    db: DbConn<'db>,
    base_url: Option<String>,
    sets: Vec<(MetaCol, BindValue)>,
    where_sql: Vec<String>,
    binds: Vec<BindValue>,
}

impl<'db> MetaUpdate<'db> {
    pub fn new(db: DbConn<'db>, base_url: Option<String>) -> Self {
        Self {
            db,
            base_url,
    ...
    pub fn unsafe_sql(self) -> MetaUnsafeUpdate<'db> {
        MetaUnsafeUpdate::new(self)
    }
    pub fn set_id(mut self, val: i64) -> Self {
        self.sets.push((MetaCol::Id, val.into()));
        self
    }
    pub fn set_owner_type(mut self, val: String) -> Self {
        self.sets.push((MetaCol::OwnerType, val.into()));
        self
    }
    pub fn set_owner_id(mut self, val: i64) -> Self {
        self.sets.push((MetaCol::OwnerId, val.into()));
        self
    }
    pub fn set_field(mut self, val: String) -> Self {
        self.sets.push((MetaCol::Field, val.into()));
        self
    }
    pub fn set_value(mut self, val: serde_json::Value) -> Self {
        self.sets.push((MetaCol::Value, val.into()));
        self
    }
    pub fn where_id(mut self, op: Op, val: i64) -> Self {
        let idx = self.binds.len() + 1;
...
#[doc(hidden)]
pub struct MetaUnsafeUpdate<'db> {
    inner: MetaUpdate<'db>,
}

impl<'db> MetaUnsafeUpdate<'db> {
    fn new(inner: MetaUpdate<'db>) -> Self {
        Self { inner }
    }
    pub fn where_raw(mut self, clause: RawClause) -> Self {
        let (sql, binds) = clause.into_parts();
    ...
    pub fn done(self) -> MetaUpdate<'db> {
        self.inner
    }
}
pub struct MetaTableAdapter;
impl MetaTableAdapter {
    fn parse_col(name: &str) -> Option<MetaCol> {
        match name {
            "id" => Some(MetaCol::Id),
            "owner_type" => Some(MetaCol::OwnerType),
    ...
    fn parse_like_col(name: &str) -> Option<MetaCol> {
        match name {
            "owner_type" => Some(MetaCol::OwnerType),
            "field" => Some(MetaCol::Field),
            _ => None,
...
impl GeneratedTableAdapter for MetaTableAdapter {
    ...
    fn apply_auto_filter<'db>(
        ...
        filter: &ParsedFilter,
        value: &str,
    ) -> anyhow::Result<Option<MetaQuery<'db>>>
    where
        Self: 'db,
    {
        ...
        Self: 'db,
    {
        ...
        let next = match column {
            "id" => query.order_by(MetaCol::Id, dir),
            "owner_type" => query.order_by(MetaCol::OwnerType, dir),
    ...
    fn cursor_from_row(&self, row: &MetaView, column: &str) -> Option<String> {
        match column {
    ...
    fn count<'db>(&self, query: MetaQuery<'db>) -> BoxFuture<'db, anyhow::Result<i64>>
    where
        Self: 'db,
    {
    ...
    fn fetch_page<'db>(
        ...
        page: i64,
        per_page: i64,
    ) -> BoxFuture<'db, anyhow::Result<Vec<MetaView>>>
    where
        Self: 'db,
    {
...
pub struct MetaDataTable<H = MetaDefaultDataTableHooks>
where
    H: MetaDataTableHooks,
{
    pub db: sqlx::PgPool,
    pub hooks: H,
...
impl<H: MetaDataTableHooks> AutoDataTable for MetaDataTable<H> {
    ...
    fn base_query<'db>(
        ...
        self.hooks
            .scope(Meta::new(&self.db, None).query(), input, ctx)
    }
...
impl ActiveRecord for MetaView {
    type Id = i64;
    async fn find(db: &sqlx::PgPool, id: Self::Id) -> anyhow::Result<Option<Self>> {
        Meta::new(db, None).find(id).await.map_err(|e| e.into())
    }
}
