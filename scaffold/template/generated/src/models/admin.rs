...
pub trait AdminViewsExt {
    ...
    fn key_by<K>(&self, f: impl Fn(&AdminView) -> K) -> std::collections::HashMap<K, AdminView>
    where
        K: Eq + std::hash::Hash;
    fn group_by<K>(
        &self,
        f: impl Fn(&AdminView) -> K,
    ) -> std::collections::HashMap<K, Vec<AdminView>>
    where
        K: Eq + std::hash::Hash;
}
...
impl<'db> Admin<'db> {
    ...
    pub fn query(&self) -> AdminQuery<'db> {
        AdminQuery::new(self.db.clone(), self.base_url.clone())
    }
    pub fn insert(&self) -> AdminInsert<'db> {
        AdminInsert::new(self.db.clone(), self.base_url.clone())
    }
    pub fn update(&self) -> AdminUpdate<'db> {
        AdminUpdate::new(self.db.clone(), self.base_url.clone())
    }
...
impl<'db> AdminQuery<'db> {
    ...
    pub fn unsafe_sql(self) -> AdminUnsafeQuery<'db> {
        AdminUnsafeQuery::new(self)
    }
    ...
    pub fn where_key_in<T: Clone + Into<BindValue>>(self, vals: &[T]) -> Self {
        self.where_in(AdminCol::Id, vals)
    }
    ...
    pub async fn first_or_fail(self) -> Result<AdminView> {
        self.first()
            .await?
            .ok_or_else(|| anyhow::anyhow!("admin: record not found"))
    }
    ...
    pub async fn first_or_create(
        self,
        create: impl FnOnce(AdminInsert<'db>) -> AdminInsert<'db>,
    ) -> Result<AdminView> {
    ...
    pub async fn update_or_create(
        ...
        if let Some(existing) = self.first().await? {
            ...
            return Admin::new(db, base_url.clone())
                .query()
                .find(existing.id)
                .await
                .map(|r| r.unwrap());
...
impl<'db> AdminUnsafeQuery<'db> {
    fn new(inner: AdminQuery<'db>) -> Self {
        Self { inner }
    }
    ...
    pub fn done(self) -> AdminQuery<'db> {
        self.inner
    }
}
...
impl<'db> AdminUpdate<'db> {
    ...
    pub fn unsafe_sql(self) -> AdminUnsafeUpdate<'db> {
        AdminUnsafeUpdate::new(self)
    }
    pub fn set_id(mut self, val: i64) -> Self {
...
impl<'db> AdminUnsafeUpdate<'db> {
    fn new(inner: AdminUpdate<'db>) -> Self {
        Self { inner }
    }
    pub fn where_raw(mut self, clause: RawClause) -> Self {
        let (sql, binds) = clause.into_parts();
    ...
    pub fn done(self) -> AdminUpdate<'db> {
        self.inner
    }
}
...
impl GeneratedTableAdapter for AdminTableAdapter {
    ...
    fn count<'db>(&self, query: AdminQuery<'db>) -> BoxFuture<'db, anyhow::Result<i64>>
    where
        Self: 'db,
    {
    ...
    fn fetch_page<'db>(
        ...
        page: i64,
        per_page: i64,
    ) -> BoxFuture<'db, anyhow::Result<Vec<AdminView>>>
    where
        Self: 'db,
    {
...
pub struct AdminDataTable<H = AdminDefaultDataTableHooks>
where
    H: AdminDataTableHooks,
{
    pub db: sqlx::PgPool,
    pub hooks: H,
...
impl<H: AdminDataTableHooks> AutoDataTable for AdminDataTable<H> {
    ...
    fn base_query<'db>(
        ...
        self.hooks
            .scope(Admin::new(&self.db, None).query(), input, ctx)
    }
