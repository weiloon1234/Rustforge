...
impl<'db> WithdrawalQuery<'db> {
    ...
    pub fn select_cols(mut self, cols: &[WithdrawalCol]) -> Self {
        if cols.is_empty() {
            ...
            let mut list: Vec<String> = "id, owner_type, owner_id, admin_id, credit_type, withdrawal_method, bank_id, bank_account_name, bank_account_number, crypto_network_id, crypto_wallet_address, conversion_rate, status, amount, fee, net_amount, related_key, params, remark, admin_remark, reviewed_at, created_at, updated_at".split(',').map(|s| s.trim().to_string()).collect();
    ...
    fn select_raw(mut self, sql: impl Into<String>) -> Self {
        ...
        if s.is_empty() {
            ...
            self.select_sql = Some(format!("id, owner_type, owner_id, admin_id, credit_type, withdrawal_method, bank_id, bank_account_name, bank_account_number, crypto_network_id, crypto_wallet_address, conversion_rate, status, amount, fee, net_amount, related_key, params, remark, admin_remark, reviewed_at, created_at, updated_at, {}", s));
        }
        self
    }
    fn add_select_raw(mut self, sql: impl Into<String>) -> Self {
        ...
        let mut base = self.select_sql.take().unwrap_or_else(|| "id, owner_type, owner_id, admin_id, credit_type, withdrawal_method, bank_id, bank_account_name, bank_account_number, crypto_network_id, crypto_wallet_address, conversion_rate, status, amount, fee, net_amount, related_key, params, remark, admin_remark, reviewed_at, created_at, updated_at".to_string());
...
pub async fn get_as<T>(self) -> Result<Vec<T>>
    ...
    pub async fn increment(self, col: WithdrawalCol, amount: i64) -> Result<u64> {
        ...
        let where_clause = if where_sql.is_empty() { String::new() } else { format!(" WHERE {}", where_sql.join(" AND ")) };
        let sql = format!("UPDATE withdrawals SET {} = {} + {} {}", col.as_sql(), col.as_sql(), amount, where_clause);
    ...
    pub async fn sum(self, col: WithdrawalCol) -> Result<Option<f64>> {
        ...
        record_profiled_query("withdrawals", "SUM", &sql, &__profiler_binds, __profiler_start.elapsed());
        Ok(result)
    }
    ...
    pub async fn avg(self, col: WithdrawalCol) -> Result<Option<f64>> {
        ...
        for b in join_binds { q = bind_scalar(q, b); }
        let result = db.fetch_scalar(q).await?;
        record_profiled_query("withdrawals", "AVG", &sql, &__profiler_binds, __profiler_start.elapsed());
        Ok(result)
    }
    ...
    pub async fn min_val(self, col: WithdrawalCol) -> Result<Option<i64>> {
        ...
        for b in join_binds { q = bind_scalar(q, b); }
        let result = db.fetch_scalar(q).await?;
        record_profiled_query("withdrawals", "MIN_VAL", &sql, &__profiler_binds, __profiler_start.elapsed());
        Ok(result)
    }
    ...
    pub async fn max_val(self, col: WithdrawalCol) -> Result<Option<i64>> {
        ...
        for b in join_binds { q = bind_scalar(q, b); }
        let result = db.fetch_scalar(q).await?;
        record_profiled_query("withdrawals", "MAX_VAL", &sql, &__profiler_binds, __profiler_start.elapsed());
        Ok(result)
    }
...
pub async fn save(self) -> Result<WithdrawalView> {
    ...
    async fn save_with_db<'tx>(self, db: DbConn<'tx>) -> Result<(WithdrawalView, WithdrawalRow)> {
        ...
        let mut sql = format!("INSERT INTO {} ({}) VALUES ({})", "withdrawals", col_sql.join(", "), placeholders.join(", "));
...
pub async fn save(self) -> Result<u64> {
    ...
    async fn save_with_db<'tx>(self, db: DbConn<'tx>, observer_changes: Option<WithdrawalUpdateChanges>) -> Result<u64> {
        ...
        let select_sql = format!("SELECT id FROM withdrawals WHERE {}", self.where_sql.join(" AND "));
        ...
        let mut sql = String::from("UPDATE withdrawals SET ");
