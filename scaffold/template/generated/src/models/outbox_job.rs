...
pub trait OutboxJobViewsExt {
    ...
    fn key_by<K>(
        &self,
        f: impl Fn(&OutboxJobView) -> K,
    ) -> std::collections::HashMap<K, OutboxJobView>
    where
        K: Eq + std::hash::Hash;
    fn group_by<K>(
        &self,
        f: impl Fn(&OutboxJobView) -> K,
    ) -> std::collections::HashMap<K, Vec<OutboxJobView>>
    where
        K: Eq + std::hash::Hash;
}
...
impl OutboxJobCol {
    pub const fn all() -> &'static [OutboxJobCol] {
        &[
            OutboxJobCol::Id,
            OutboxJobCol::Queue,
            OutboxJobCol::Payload,
            OutboxJobCol::CreatedAt,
        ]
    }
...
impl<'db> OutboxJob<'db> {
    ...
    pub fn query(&self) -> OutboxJobQuery<'db> {
        OutboxJobQuery::new(self.db.clone(), self.base_url.clone())
    }
    pub fn insert(&self) -> OutboxJobInsert<'db> {
        OutboxJobInsert::new(self.db.clone(), self.base_url.clone())
    }
    pub fn update(&self) -> OutboxJobUpdate<'db> {
        OutboxJobUpdate::new(self.db.clone(), self.base_url.clone())
    }
...
impl<'db> OutboxJobQuery<'db> {
    pub fn new(db: DbConn<'db>, base_url: Option<String>) -> Self {
        Self {
            db,
            base_url,
            select_sql: Some("id, queue, payload, created_at".to_string()),
            from_sql: None,
            count_sql: None,
            distinct: false,
            distinct_on: None,
    ...
    pub fn where_key_in<T: Clone + Into<BindValue>>(self, vals: &[T]) -> Self {
        self.where_in(OutboxJobCol::Id, vals)
    }
    ...
    pub fn select_cols(mut self, cols: &[OutboxJobCol]) -> Self {
        if cols.is_empty() {
            ...
            let mut list: Vec<String> = "id, queue, payload, created_at"
                .split(',')
    ...
    fn add_select_raw(mut self, sql: impl Into<String>) -> Self {
        ...
        let mut base = self
            .select_sql
            .take()
            .unwrap_or_else(|| "id, queue, payload, created_at".to_string());
        if !base.is_empty() {
            base.push_str(", ");
        }
        base.push_str(&s);
    ...
    pub async fn first_or_fail(self) -> Result<OutboxJobView> {
        self.first()
            .await?
            .ok_or_else(|| anyhow::anyhow!("outbox_jobs: record not found"))
    }
    ...
    pub async fn first_or_create(
        self,
        create: impl FnOnce(OutboxJobInsert<'db>) -> OutboxJobInsert<'db>,
    ...
    pub async fn update_or_create(
        ...
        if let Some(existing) = self.first().await? {
            ...
            return OutboxJob::new(db, base_url.clone())
                .query()
                .find(existing.id)
                .await
                .map(|r| r.unwrap());
    ...
    pub async fn pluck_pair<K, V>(
        self,
        extract: impl Fn(&OutboxJobView) -> (K, V),
...
impl<'db> OutboxJobUnsafeQuery<'db> {
    fn new(inner: OutboxJobQuery<'db>) -> Self {
        Self { inner }
    }
    ...
    pub fn done(self) -> OutboxJobQuery<'db> {
        self.inner
    }
}
...
impl<'db> OutboxJobUpdate<'db> {
    ...
    pub fn unsafe_sql(self) -> OutboxJobUnsafeUpdate<'db> {
        OutboxJobUnsafeUpdate::new(self)
    }
    pub fn set_id(mut self, val: uuid::Uuid) -> Self {
...
impl<'db> OutboxJobUnsafeUpdate<'db> {
    fn new(inner: OutboxJobUpdate<'db>) -> Self {
        Self { inner }
    }
    pub fn where_raw(mut self, clause: RawClause) -> Self {
        let (sql, binds) = clause.into_parts();
    ...
    pub fn done(self) -> OutboxJobUpdate<'db> {
        self.inner
    }
}
...
impl GeneratedTableAdapter for OutboxJobTableAdapter {
    ...
    fn apply_auto_filter<'db>(
        &self,
        query: OutboxJobQuery<'db>,
        filter: &ParsedFilter,
        value: &str,
    ) -> anyhow::Result<Option<OutboxJobQuery<'db>>>
    where
        Self: 'db,
    {
    ...
    fn apply_sort<'db>(
        ...
        column: &str,
        dir: SortDirection,
    ) -> anyhow::Result<OutboxJobQuery<'db>>
    where
        Self: 'db,
    {
        let dir = match dir {
    ...
    fn count<'db>(&self, query: OutboxJobQuery<'db>) -> BoxFuture<'db, anyhow::Result<i64>>
    where
        Self: 'db,
    {
    ...
    fn fetch_page<'db>(
        ...
        page: i64,
        per_page: i64,
    ) -> BoxFuture<'db, anyhow::Result<Vec<OutboxJobView>>>
    where
        Self: 'db,
    {
...
pub struct OutboxJobDataTable<H = OutboxJobDefaultDataTableHooks>
where
    H: OutboxJobDataTableHooks,
{
    pub db: sqlx::PgPool,
    pub hooks: H,
...
impl<H: OutboxJobDataTableHooks> AutoDataTable for OutboxJobDataTable<H> {
    ...
    fn base_query<'db>(
        ...
        self.hooks
            .scope(OutboxJob::new(&self.db, None).query(), input, ctx)
    }
