...
pub trait WebhookLogViewsExt {
    ...
    fn key_by<K>(
        &self,
        f: impl Fn(&WebhookLogView) -> K,
    ) -> std::collections::HashMap<K, WebhookLogView>
    where
        K: Eq + std::hash::Hash;
    fn group_by<K>(
        &self,
        f: impl Fn(&WebhookLogView) -> K,
    ) -> std::collections::HashMap<K, Vec<WebhookLogView>>
    where
        K: Eq + std::hash::Hash;
}
...
fn hydrate_view(row: WebhookLogRow, loc: &LocalizedMap, base_url: Option<&str>) -> WebhookLogView {
    let mut view = WebhookLogView {
        id: row.id,
...
impl WebhookLogCol {
    pub const fn all() -> &'static [WebhookLogCol] {
        &[
            WebhookLogCol::Id,
            WebhookLogCol::RequestUrl,
            WebhookLogCol::RequestMethod,
            WebhookLogCol::ResponseStatus,
            WebhookLogCol::DurationMs,
            WebhookLogCol::RequestHeaders,
            WebhookLogCol::RequestBody,
            WebhookLogCol::ResponseBody,
            WebhookLogCol::CreatedAt,
        ]
    }
...
impl<'db> WebhookLog<'db> {
    ...
    pub fn query(&self) -> WebhookLogQuery<'db> {
        WebhookLogQuery::new(self.db.clone(), self.base_url.clone())
    }
    pub fn insert(&self) -> WebhookLogInsert<'db> {
        WebhookLogInsert::new(self.db.clone(), self.base_url.clone())
    }
    pub fn update(&self) -> WebhookLogUpdate<'db> {
        WebhookLogUpdate::new(self.db.clone(), self.base_url.clone())
    }
...
impl<'db> WebhookLogQuery<'db> {
    ...
    pub fn where_request_url(mut self, op: Op, val: String) -> Self {
        ...
        self.where_sql.push(format!(
            "{} {} ${}",
            WebhookLogCol::RequestUrl.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_request_method(mut self, op: Op, val: String) -> Self {
        ...
        self.where_sql.push(format!(
            "{} {} ${}",
            WebhookLogCol::RequestMethod.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_response_status(mut self, op: Op, val: i32) -> Self {
        ...
        self.where_sql.push(format!(
            "{} {} ${}",
            WebhookLogCol::ResponseStatus.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_duration_ms(mut self, op: Op, val: i32) -> Self {
        ...
        self.where_sql.push(format!(
            "{} {} ${}",
            WebhookLogCol::DurationMs.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_request_headers(mut self, op: Op, val: serde_json::Value) -> Self {
        ...
        self.where_sql.push(format!(
            "{} {} ${}",
            WebhookLogCol::RequestHeaders.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_request_body(mut self, op: Op, val: Option<String>) -> Self {
        ...
        self.where_sql.push(format!(
            "{} {} ${}",
            WebhookLogCol::RequestBody.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_response_body(mut self, op: Op, val: Option<String>) -> Self {
        ...
        self.where_sql.push(format!(
            "{} {} ${}",
            WebhookLogCol::ResponseBody.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_created_at(mut self, op: Op, val: time::OffsetDateTime) -> Self {
        ...
        self.where_sql.push(format!(
            "{} {} ${}",
            WebhookLogCol::CreatedAt.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_key_in<T: Clone + Into<BindValue>>(self, vals: &[T]) -> Self {
        self.where_in(WebhookLogCol::Id, vals)
    }
    ...
    pub fn select_cols(mut self, cols: &[WebhookLogCol]) -> Self {
        if cols.is_empty() {
            self.select_sql = Some("id, request_url, request_method, response_status, duration_ms, request_headers, request_body, response_body, created_at".to_string());
        } else {
            let mut seen = std::collections::BTreeSet::new();
            let mut list: Vec<String> = "id, request_url, request_method, response_status, duration_ms, request_headers, request_body, response_body, created_at".split(',').map(|s| s.trim().to_string()).collect();
            for s in &list {
                seen.insert(s.clone());
            }
            for c in cols {
    ...
    pub fn add_select_cols(mut self, cols: &[WebhookLogCol]) -> Self {
        ...
        let mut list: Vec<String> = match self.select_sql.take() {
            ...
            _ => "id, request_url, request_method, response_status, duration_ms, request_headers, request_body, response_body, created_at".split(',').map(|s| s.trim().to_string()).collect(),
        };
        for s in &list {
            seen.insert(s.clone());
        }
    ...
    fn select_raw(mut self, sql: impl Into<String>) -> Self {
        ...
        if s.is_empty() {
            ...
            self.select_sql = Some(format!("id, request_url, request_method, response_status, duration_ms, request_headers, request_body, response_body, created_at, {}", s));
        }
        self
    }
    fn add_select_raw(mut self, sql: impl Into<String>) -> Self {
        ...
        let mut base = self.select_sql.take().unwrap_or_else(|| "id, request_url, request_method, response_status, duration_ms, request_headers, request_body, response_body, created_at".to_string());
        if !base.is_empty() {
            base.push_str(", ");
        }
        base.push_str(&s);
    ...
    pub async fn first_or_fail(self) -> Result<WebhookLogView> {
        self.first()
            .await?
            .ok_or_else(|| anyhow::anyhow!("webhook_logs: record not found"))
    }
    ...
    pub async fn first_or_create(
        self,
        create: impl FnOnce(WebhookLogInsert<'db>) -> WebhookLogInsert<'db>,
    ...
    pub async fn update_or_create(
        ...
        if let Some(existing) = self.first().await? {
            ...
            return WebhookLog::new(db, base_url.clone())
                .query()
                .find(existing.id)
                .await
                .map(|r| r.unwrap());
    ...
    pub async fn pluck_pair<K, V>(
        self,
        extract: impl Fn(&WebhookLogView) -> (K, V),
...
impl<'db> WebhookLogUnsafeQuery<'db> {
    fn new(inner: WebhookLogQuery<'db>) -> Self {
        Self { inner }
    }
    ...
    pub fn done(self) -> WebhookLogQuery<'db> {
        self.inner
    }
}
...
impl<'db> WebhookLogUpdate<'db> {
    ...
    pub fn unsafe_sql(self) -> WebhookLogUnsafeUpdate<'db> {
        WebhookLogUnsafeUpdate::new(self)
    }
    pub fn set_id(mut self, val: uuid::Uuid) -> Self {
    ...
    async fn save_with_db<'tx>(self, db: DbConn<'tx>) -> Result<u64> {
        ...
        let select_sql = format!(
            "SELECT id FROM webhook_logs WHERE {}",
...
impl<'db> WebhookLogUnsafeUpdate<'db> {
    fn new(inner: WebhookLogUpdate<'db>) -> Self {
        Self { inner }
    }
    pub fn where_raw(mut self, clause: RawClause) -> Self {
        let (sql, binds) = clause.into_parts();
    ...
    pub fn done(self) -> WebhookLogUpdate<'db> {
        self.inner
    }
}
...
impl GeneratedTableAdapter for WebhookLogTableAdapter {
    ...
    fn apply_auto_filter<'db>(
        &self,
        query: WebhookLogQuery<'db>,
        filter: &ParsedFilter,
        value: &str,
    ) -> anyhow::Result<Option<WebhookLogQuery<'db>>>
    where
        Self: 'db,
    {
    ...
    fn apply_sort<'db>(
        ...
        column: &str,
        dir: SortDirection,
    ) -> anyhow::Result<WebhookLogQuery<'db>>
    where
        Self: 'db,
    {
        let dir = match dir {
    ...
    fn count<'db>(&self, query: WebhookLogQuery<'db>) -> BoxFuture<'db, anyhow::Result<i64>>
    where
        Self: 'db,
    {
    ...
    fn fetch_page<'db>(
        ...
        page: i64,
        per_page: i64,
    ) -> BoxFuture<'db, anyhow::Result<Vec<WebhookLogView>>>
    where
        Self: 'db,
    {
...
pub struct WebhookLogDataTable<H = WebhookLogDefaultDataTableHooks>
where
    H: WebhookLogDataTableHooks,
{
    pub db: sqlx::PgPool,
    pub hooks: H,
...
impl<H: WebhookLogDataTableHooks> AutoDataTable for WebhookLogDataTable<H> {
    ...
    fn base_query<'db>(
        ...
        self.hooks
            .scope(WebhookLog::new(&self.db, None).query(), input, ctx)
    }
