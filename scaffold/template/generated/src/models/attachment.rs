...
pub trait AttachmentViewsExt {
    ...
    fn key_by<K>(
        &self,
        f: impl Fn(&AttachmentView) -> K,
    ) -> std::collections::HashMap<K, AttachmentView>
    where
        K: Eq + std::hash::Hash;
    fn group_by<K>(
        &self,
        f: impl Fn(&AttachmentView) -> K,
    ) -> std::collections::HashMap<K, Vec<AttachmentView>>
    where
        K: Eq + std::hash::Hash;
}
...
impl<'db> Attachment<'db> {
    ...
    pub fn query(&self) -> AttachmentQuery<'db> {
        AttachmentQuery::new(self.db.clone(), self.base_url.clone())
    }
    pub fn insert(&self) -> AttachmentInsert<'db> {
        AttachmentInsert::new(self.db.clone(), self.base_url.clone())
    }
    pub fn update(&self) -> AttachmentUpdate<'db> {
        AttachmentUpdate::new(self.db.clone(), self.base_url.clone())
    }
...
impl<'db> AttachmentQuery<'db> {
    ...
    pub fn unsafe_sql(self) -> AttachmentUnsafeQuery<'db> {
        AttachmentUnsafeQuery::new(self)
    }
    pub fn where_id(mut self, op: Op, val: uuid::Uuid) -> Self {
        let idx = self.binds.len() + 1;
        self.where_sql.push(format!(
            "{} {} ${}",
            AttachmentCol::Id.as_sql(),
            op.as_sql(),
            idx
        ));
    ...
    pub fn where_owner_type(mut self, op: Op, val: String) -> Self {
        ...
        self.where_sql.push(format!(
            "{} {} ${}",
            AttachmentCol::OwnerType.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_content_type(mut self, op: Op, val: String) -> Self {
        ...
        self.where_sql.push(format!(
            "{} {} ${}",
            AttachmentCol::ContentType.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_created_at(mut self, op: Op, val: time::OffsetDateTime) -> Self {
        ...
        self.where_sql.push(format!(
            "{} {} ${}",
            AttachmentCol::CreatedAt.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_updated_at(mut self, op: Op, val: time::OffsetDateTime) -> Self {
        ...
        self.where_sql.push(format!(
            "{} {} ${}",
            AttachmentCol::UpdatedAt.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_deleted_at(mut self, op: Op, val: Option<time::OffsetDateTime>) -> Self {
        ...
        self.where_sql.push(format!(
            "{} {} ${}",
            AttachmentCol::DeletedAt.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_key_in<T: Clone + Into<BindValue>>(self, vals: &[T]) -> Self {
        self.where_in(AttachmentCol::Id, vals)
    }
    ...
    pub async fn first_or_fail(self) -> Result<AttachmentView> {
        self.first()
            .await?
            .ok_or_else(|| anyhow::anyhow!("attachments: record not found"))
    }
    ...
    pub async fn first_or_create(
        self,
        create: impl FnOnce(AttachmentInsert<'db>) -> AttachmentInsert<'db>,
    ...
    pub async fn update_or_create(
        ...
        if let Some(existing) = self.first().await? {
            ...
            return Attachment::new(db, base_url.clone())
                .query()
                .find(existing.id)
                .await
                .map(|r| r.unwrap());
    ...
    pub async fn pluck_pair<K, V>(
        self,
        extract: impl Fn(&AttachmentView) -> (K, V),
...
impl<'db> AttachmentUnsafeQuery<'db> {
    fn new(inner: AttachmentQuery<'db>) -> Self {
        Self { inner }
    }
    ...
    pub fn done(self) -> AttachmentQuery<'db> {
        self.inner
    }
}
...
impl<'db> AttachmentUpdate<'db> {
    ...
    pub fn unsafe_sql(self) -> AttachmentUnsafeUpdate<'db> {
        AttachmentUnsafeUpdate::new(self)
    }
    pub fn set_id(mut self, val: uuid::Uuid) -> Self {
    ...
    async fn save_with_db<'tx>(self, db: DbConn<'tx>) -> Result<u64> {
        ...
        let select_sql = format!(
            "SELECT id FROM attachments WHERE {}",
...
impl<'db> AttachmentUnsafeUpdate<'db> {
    fn new(inner: AttachmentUpdate<'db>) -> Self {
        Self { inner }
    }
    pub fn where_raw(mut self, clause: RawClause) -> Self {
        let (sql, binds) = clause.into_parts();
    ...
    pub fn done(self) -> AttachmentUpdate<'db> {
        self.inner
    }
}
...
impl GeneratedTableAdapter for AttachmentTableAdapter {
    ...
    fn apply_auto_filter<'db>(
        &self,
        query: AttachmentQuery<'db>,
        filter: &ParsedFilter,
        value: &str,
    ) -> anyhow::Result<Option<AttachmentQuery<'db>>>
    where
        Self: 'db,
    {
    ...
    fn apply_sort<'db>(
        ...
        column: &str,
        dir: SortDirection,
    ) -> anyhow::Result<AttachmentQuery<'db>>
    where
        Self: 'db,
    {
        let dir = match dir {
    ...
    fn count<'db>(&self, query: AttachmentQuery<'db>) -> BoxFuture<'db, anyhow::Result<i64>>
    where
        Self: 'db,
    {
    ...
    fn fetch_page<'db>(
        ...
        page: i64,
        per_page: i64,
    ) -> BoxFuture<'db, anyhow::Result<Vec<AttachmentView>>>
    where
        Self: 'db,
    {
...
pub struct AttachmentDataTable<H = AttachmentDefaultDataTableHooks>
where
    H: AttachmentDataTableHooks,
{
    pub db: sqlx::PgPool,
    pub hooks: H,
...
impl<H: AttachmentDataTableHooks> AutoDataTable for AttachmentDataTable<H> {
    ...
    fn base_query<'db>(
        ...
        self.hooks
            .scope(Attachment::new(&self.db, None).query(), input, ctx)
    }
