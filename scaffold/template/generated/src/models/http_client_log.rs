...
use core_db::common::sql::{
    bind, bind_query, bind_scalar, generate_snowflake_i64, BindValue, DbConn, Op, OrderDir,
    RawClause, RawGroupExpr, RawJoinKind, RawJoinSpec, RawOrderExpr, RawSelectExpr,
};
...
impl HttpClientLogView {
    pub fn update<'db>(&self, db: impl Into<DbConn<'db>>) -> HttpClientLogUpdate<'db> {
        HttpClientLog::new(db.into(), None)
            .update()
            .where_id(Op::Eq, self.id)
    }
...
pub trait HttpClientLogViewsExt {
    ...
    fn key_by<K>(
        &self,
        f: impl Fn(&HttpClientLogView) -> K,
    ) -> std::collections::HashMap<K, HttpClientLogView>
    where
        K: Eq + std::hash::Hash;
    fn group_by<K>(
        &self,
        f: impl Fn(&HttpClientLogView) -> K,
    ) -> std::collections::HashMap<K, Vec<HttpClientLogView>>
    where
        K: Eq + std::hash::Hash;
}
...
fn hydrate_view(
    row: HttpClientLogRow,
    loc: &LocalizedMap,
    base_url: Option<&str>,
) -> HttpClientLogView {
    let mut view = HttpClientLogView {
        id: row.id,
...
impl<'db> HttpClientLog<'db> {
    ...
    pub fn query(&self) -> HttpClientLogQuery<'db> {
        HttpClientLogQuery::new(self.db.clone(), self.base_url.clone())
    }
    pub fn insert(&self) -> HttpClientLogInsert<'db> {
        HttpClientLogInsert::new(self.db.clone(), self.base_url.clone())
    }
    pub fn update(&self) -> HttpClientLogUpdate<'db> {
        HttpClientLogUpdate::new(self.db.clone(), self.base_url.clone())
    }
    pub async fn find(&self, id: uuid::Uuid) -> Result<Option<HttpClientLogView>> {
...
impl<'db> HttpClientLogQuery<'db> {
    pub fn new(db: DbConn<'db>, base_url: Option<String>) -> Self {
        Self { db, base_url, select_sql: Some("id, request_url, request_method, response_status, duration_ms, request_headers, request_body, response_headers, response_body, created_at".to_string()), from_sql: None, count_sql: None, distinct: false, distinct_on: None, lock_sql: None, join_sql: vec![], join_binds: vec![], where_sql: vec![], order_sql: vec![], group_by_sql: vec![], having_sql: vec![], having_binds: vec![], offset: None, limit: None, binds: vec![] }
    }
    pub fn unsafe_sql(self) -> HttpClientLogUnsafeQuery<'db> {
        HttpClientLogUnsafeQuery::new(self)
    }
    pub fn where_id(mut self, op: Op, val: uuid::Uuid) -> Self {
        let idx = self.binds.len() + 1;
        self.where_sql.push(format!(
            "{} {} ${}",
            HttpClientLogCol::Id.as_sql(),
            op.as_sql(),
            idx
        ));
    ...
    pub fn where_request_url(mut self, op: Op, val: String) -> Self {
        ...
        self.where_sql.push(format!(
            "{} {} ${}",
            HttpClientLogCol::RequestUrl.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_request_method(mut self, op: Op, val: String) -> Self {
        ...
        self.where_sql.push(format!(
            "{} {} ${}",
            HttpClientLogCol::RequestMethod.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_response_status(mut self, op: Op, val: i32) -> Self {
        let idx = self.binds.len() + 1;
        self.where_sql.push(format!(
            "{} {} ${}",
            HttpClientLogCol::ResponseStatus.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_duration_ms(mut self, op: Op, val: i32) -> Self {
        let idx = self.binds.len() + 1;
        self.where_sql.push(format!(
            "{} {} ${}",
            HttpClientLogCol::DurationMs.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_request_headers(mut self, op: Op, val: serde_json::Value) -> Self {
        let idx = self.binds.len() + 1;
        self.where_sql.push(format!(
            "{} {} ${}",
            HttpClientLogCol::RequestHeaders.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_request_body(mut self, op: Op, val: Option<String>) -> Self {
        ...
        self.where_sql.push(format!(
            "{} {} ${}",
            HttpClientLogCol::RequestBody.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_response_headers(mut self, op: Op, val: serde_json::Value) -> Self {
        let idx = self.binds.len() + 1;
        self.where_sql.push(format!(
            "{} {} ${}",
            HttpClientLogCol::ResponseHeaders.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_response_body(mut self, op: Op, val: Option<String>) -> Self {
        ...
        self.where_sql.push(format!(
            "{} {} ${}",
            HttpClientLogCol::ResponseBody.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_created_at(mut self, op: Op, val: time::OffsetDateTime) -> Self {
        ...
        self.where_sql.push(format!(
            "{} {} ${}",
            HttpClientLogCol::CreatedAt.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_key_in<T: Clone + Into<BindValue>>(self, vals: &[T]) -> Self {
        self.where_in(HttpClientLogCol::Id, vals)
    }
    ...
    pub fn where_in<T: Clone + Into<BindValue>>(
        mut self,
        col: HttpClientLogCol,
        vals: &[T],
    ) -> Self {
        if vals.is_empty() {
    ...
    pub fn where_between<T: Into<BindValue>>(
        mut self,
        col: HttpClientLogCol,
        low: T,
        high: T,
    ...
    pub fn or_where_col<T: Into<BindValue>>(
        mut self,
        col: HttpClientLogCol,
        op: Op,
        val: T,
    ...
    pub fn select_cols(mut self, cols: &[HttpClientLogCol]) -> Self {
        if cols.is_empty() {
            self.select_sql = Some("id, request_url, request_method, response_status, duration_ms, request_headers, request_body, response_headers, response_body, created_at".to_string());
        } else {
            let mut seen = std::collections::BTreeSet::new();
            let mut list: Vec<String> = "id, request_url, request_method, response_status, duration_ms, request_headers, request_body, response_headers, response_body, created_at".split(',').map(|s| s.trim().to_string()).collect();
            for s in &list {
                seen.insert(s.clone());
            }
            for c in cols {
    ...
    pub fn add_select_cols(mut self, cols: &[HttpClientLogCol]) -> Self {
        ...
        let mut list: Vec<String> = match self.select_sql.take() {
            ...
            _ => "id, request_url, request_method, response_status, duration_ms, request_headers, request_body, response_headers, response_body, created_at".split(',').map(|s| s.trim().to_string()).collect(),
        };
        for s in &list {
            seen.insert(s.clone());
        }
    ...
    fn select_raw(mut self, sql: impl Into<String>) -> Self {
        ...
        if s.is_empty() {
            ...
            self.select_sql = Some(format!("id, request_url, request_method, response_status, duration_ms, request_headers, request_body, response_headers, response_body, created_at, {}", s));
        }
        self
    }
    fn add_select_raw(mut self, sql: impl Into<String>) -> Self {
        ...
        let mut base = self.select_sql.take().unwrap_or_else(|| "id, request_url, request_method, response_status, duration_ms, request_headers, request_body, response_headers, response_body, created_at".to_string());
        if !base.is_empty() {
            base.push_str(", ");
        }
        base.push_str(&s);
    ...
    pub async fn get(self) -> Result<Vec<HttpClientLogView>> {
        let Self {
            db,
            base_url,
            select_sql,
    ...
    pub async fn first(self) -> Result<Option<HttpClientLogView>> {
        let mut v = self.limit(1).get().await?;
        Ok(v.pop())
    }

    pub async fn first_or_fail(self) -> Result<HttpClientLogView> {
        self.first()
            .await?
            .ok_or_else(|| anyhow::anyhow!("http_client_logs: record not found"))
    }

    pub async fn find(self, id: uuid::Uuid) -> Result<Option<HttpClientLogView>> {
        self.where_id(Op::Eq, id).first().await
    }
    pub async fn find_or_fail(self, id: uuid::Uuid) -> Result<HttpClientLogView> {
        self.find(id)
            .await?
            .ok_or_else(|| anyhow::anyhow!("http_client_logs: record not found"))
    }
    pub async fn first_or_create(
        self,
        create: impl FnOnce(HttpClientLogInsert<'db>) -> HttpClientLogInsert<'db>,
    ) -> Result<HttpClientLogView> {
        let db = self.db.clone();
        let base_url = self.base_url.clone();
        if let Some(existing) = self.first().await? {
            return Ok(existing);
        }
        let insert_builder = create(HttpClientLogInsert::new(db, base_url));
    ...
    pub async fn update_or_create(
        ...
        if let Some(existing) = self.first().await? {
            ...
            return HttpClientLog::new(db, base_url.clone())
                .query()
                .find(existing.id)
                .await
                .map(|r| r.unwrap());
        }
        let insert_builder = on_create(HttpClientLogInsert::new(db, base_url));
        insert_builder.save().await
    }
    ...
    where
        F: FnMut(Vec<HttpClientLogView>) -> Fut,
    ...
    pub async fn sole(self) -> Result<HttpClientLogView> {
    ...
    pub async fn pluck_pair<K, V>(
        self,
        extract: impl Fn(&HttpClientLogView) -> (K, V),
    ...
    pub async fn paginate(self, page: i64, per_page: i64) -> Result<Page<HttpClientLogView>> {
...
impl<'db> HttpClientLogUnsafeQuery<'db> {
    fn new(inner: HttpClientLogQuery<'db>) -> Self {
        Self { inner }
    }
    ...
    pub fn done(self) -> HttpClientLogQuery<'db> {
        self.inner
    }
}
...
impl<'db> HttpClientLogInsert<'db> {
    ...
    pub fn set_response_status(mut self, val: i32) -> Self {
        self.cols.push(HttpClientLogCol::ResponseStatus);
        self.binds.push(val.into());
        self
    }
    pub fn set_duration_ms(mut self, val: i32) -> Self {
        self.cols.push(HttpClientLogCol::DurationMs);
        self.binds.push(val.into());
        self
    }
    pub fn set_request_headers(mut self, val: serde_json::Value) -> Self {
    ...
    pub fn set_response_headers(mut self, val: serde_json::Value) -> Self {
    ...
    async fn save_with_db<'tx>(self, db: DbConn<'tx>) -> Result<HttpClientLogView> {
        let mut cols = self.cols;
        let mut binds = self.binds;
        if HAS_CREATED_AT
            && !cols
                .iter()
                .any(|c| matches!(c, HttpClientLogCol::CreatedAt))
        {
...
pub struct HttpClientLogUpdate<'db> {
    db: DbConn<'db>,
    base_url: Option<String>,
    sets: Vec<(HttpClientLogCol, BindValue)>,
    where_sql: Vec<String>,
    binds: Vec<BindValue>,
}
...
impl<'db> HttpClientLogUpdate<'db> {
    ...
    pub fn unsafe_sql(self) -> HttpClientLogUnsafeUpdate<'db> {
        HttpClientLogUnsafeUpdate::new(self)
    }
    pub fn set_id(mut self, val: uuid::Uuid) -> Self {
        self.sets.push((HttpClientLogCol::Id, val.into()));
        self
    }
    pub fn set_request_url(mut self, val: String) -> Self {
        self.sets.push((HttpClientLogCol::RequestUrl, val.into()));
        self
    }
    pub fn set_request_method(mut self, val: String) -> Self {
        self.sets
            .push((HttpClientLogCol::RequestMethod, val.into()));
        self
    }
    pub fn set_response_status(mut self, val: i32) -> Self {
        self.sets
            .push((HttpClientLogCol::ResponseStatus, val.into()));
        self
    }
    pub fn set_duration_ms(mut self, val: i32) -> Self {
        self.sets.push((HttpClientLogCol::DurationMs, val.into()));
        self
    }
    pub fn set_request_headers(mut self, val: serde_json::Value) -> Self {
        self.sets
            .push((HttpClientLogCol::RequestHeaders, val.into()));
        self
    }
    pub fn set_request_body(mut self, val: Option<String>) -> Self {
        self.sets.push((HttpClientLogCol::RequestBody, val.into()));
        self
    }
    pub fn set_response_headers(mut self, val: serde_json::Value) -> Self {
        self.sets
            .push((HttpClientLogCol::ResponseHeaders, val.into()));
        self
    }
    pub fn set_response_body(mut self, val: Option<String>) -> Self {
        self.sets.push((HttpClientLogCol::ResponseBody, val.into()));
        self
    }
    pub fn set_created_at(mut self, val: time::OffsetDateTime) -> Self {
        self.sets.push((HttpClientLogCol::CreatedAt, val.into()));
        self
    }
    pub fn where_id(mut self, op: Op, val: uuid::Uuid) -> Self {
        let idx = self.binds.len() + 1;
    ...
    async fn save_with_db<'tx>(self, db: DbConn<'tx>) -> Result<u64> {
        ...
        let select_sql = format!(
            "SELECT id FROM http_client_logs WHERE {}",
        ...
        let mut sql = String::from("UPDATE http_client_logs SET ");
...
impl<'db> HttpClientLogUnsafeUpdate<'db> {
    fn new(inner: HttpClientLogUpdate<'db>) -> Self {
        Self { inner }
    }
    pub fn where_raw(mut self, clause: RawClause) -> Self {
        let (sql, binds) = clause.into_parts();
    ...
    pub fn done(self) -> HttpClientLogUpdate<'db> {
        self.inner
    }
}
...
impl HttpClientLogTableAdapter {
    ...
    fn parse_bind_for_col(name: &str, raw: &str) -> Option<BindValue> {
        match name {
            ...
            "response_status" => raw.trim().parse::<i32>().ok().map(Into::into),
            "duration_ms" => raw.trim().parse::<i32>().ok().map(Into::into),
...
impl GeneratedTableAdapter for HttpClientLogTableAdapter {
    ...
    fn column_descriptors(&self) -> &'static [DataTableColumnDescriptor] {
        &[
            ...
            DataTableColumnDescriptor {
                name: "created_at",
                label: "Created At",
                data_type: "time::OffsetDateTime",
                sortable: true,
                localized: false,
                filter_ops: &["eq", "gte", "lte", "date_from", "date_to"],
            },
        ]
    }
    ...
    fn apply_auto_filter<'db>(
        &self,
        query: HttpClientLogQuery<'db>,
        filter: &ParsedFilter,
        value: &str,
    ) -> anyhow::Result<Option<HttpClientLogQuery<'db>>>
    where
        Self: 'db,
    {
    ...
    fn apply_sort<'db>(
        ...
        column: &str,
        dir: SortDirection,
    ) -> anyhow::Result<HttpClientLogQuery<'db>>
    where
        Self: 'db,
    {
        let dir = match dir {
    ...
    fn cursor_from_row(&self, row: &HttpClientLogView, column: &str) -> Option<String> {
        match column {
            "id" => Some(row.id.to_string()),
            "request_url" => Some(row.request_url.clone()),
            "request_method" => Some(row.request_method.clone()),
            "response_status" => Some(row.response_status.to_string()),
            "duration_ms" => Some(row.duration_ms.to_string()),
    ...
    fn count<'db>(&self, query: HttpClientLogQuery<'db>) -> BoxFuture<'db, anyhow::Result<i64>>
    where
        Self: 'db,
    {
    ...
    fn fetch_page<'db>(
        ...
        page: i64,
        per_page: i64,
    ) -> BoxFuture<'db, anyhow::Result<Vec<HttpClientLogView>>>
    where
        Self: 'db,
    {
...
pub trait HttpClientLogDataTableHooks: Send + Sync + 'static {
    ...
    fn mappings(
        &self,
        _record: &mut serde_json::Map<String, serde_json::Value>,
        _input: &DataTableInput,
        _ctx: &DataTableContext,
    ) -> anyhow::Result<()> {
        Ok(())
...
pub struct HttpClientLogDataTable<H = HttpClientLogDefaultDataTableHooks>
where
    H: HttpClientLogDataTableHooks,
{
    pub db: sqlx::PgPool,
    pub hooks: H,
...
impl<H: HttpClientLogDataTableHooks> HttpClientLogDataTable<H> {
    pub fn with_hooks<NH: HttpClientLogDataTableHooks>(
        self,
        hooks: NH,
    ) -> HttpClientLogDataTable<NH> {
...
impl<H: HttpClientLogDataTableHooks> AutoDataTable for HttpClientLogDataTable<H> {
    ...
    fn base_query<'db>(
        ...
        self.hooks
            .scope(HttpClientLog::new(&self.db, None).query(), input, ctx)
    }
    ...
    fn mappings(
        ...
        self.hooks.mappings(record, input, ctx)
    }
    fn summary<'db>(
        &'db self,
        query: HttpClientLogQuery<'db>,
