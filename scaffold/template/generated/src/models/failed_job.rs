...
impl<'db> FailedJobQuery<'db> {
    ...
    pub async fn first_or_create(self, create: impl FnOnce(FailedJobInsert<'db>) -> FailedJobInsert<'db>) -> Result<FailedJobView> {
    ...
    pub async fn update_or_create(
        ...
        if let Some(existing) = self.first().await? {
            ...
            return FailedJob::new(db, base_url.clone()).query().find(existing.id).await.map(|r| r.unwrap());
        }
    ...
    pub async fn pluck_pair<K, V>(self, extract: impl Fn(&FailedJobView) -> (K, V)) -> Result<std::collections::HashMap<K, V>>
    where
        K: Eq + std::hash::Hash,
    {
...
impl GeneratedTableAdapter for FailedJobTableAdapter {
    ...
    fn fetch_page<'db>(&self, query: FailedJobQuery<'db>, page: i64, per_page: i64) -> BoxFuture<'db, anyhow::Result<Vec<FailedJobView>>> where Self: 'db {
...
pub trait FailedJobDataTableHooks: Send + Sync + 'static {
    ...
    fn map_row(&self, _row: &mut FailedJobView, _input: &DataTableInput, _ctx: &DataTableContext) -> anyhow::Result<()> { Ok(()) }
    fn default_row_to_record(&self, row: FailedJobView) -> anyhow::Result<serde_json::Map<String, serde_json::Value>> {
    ...
    fn row_to_record(&self, row: FailedJobView, _input: &DataTableInput, _ctx: &DataTableContext) -> anyhow::Result<serde_json::Map<String, serde_json::Value>> {
...
impl<H: FailedJobDataTableHooks> AutoDataTable for FailedJobDataTable<H> {
    ...
    fn map_row(&self, row: &mut FailedJobView, input: &DataTableInput, ctx: &DataTableContext) -> anyhow::Result<()> { self.hooks.map_row(row, input, ctx) }
    fn row_to_record(&self, row: FailedJobView, input: &DataTableInput, ctx: &DataTableContext) -> anyhow::Result<serde_json::Map<String, serde_json::Value>> { self.hooks.row_to_record(row, input, ctx) }
...
impl ActiveRecord for FailedJobView {
    ...
    async fn find(db: &sqlx::PgPool, id: Self::Id) -> anyhow::Result<Option<Self>> {
        FailedJob::new(db, None).find(id).await.map_err(|e| e.into())
    }
}
