...
pub trait FailedJobViewsExt {
    ...
    fn key_by<K>(
        &self,
        f: impl Fn(&FailedJobView) -> K,
    ) -> std::collections::HashMap<K, FailedJobView>
    where
        K: Eq + std::hash::Hash;
    fn group_by<K>(
        &self,
        f: impl Fn(&FailedJobView) -> K,
    ) -> std::collections::HashMap<K, Vec<FailedJobView>>
    where
        K: Eq + std::hash::Hash;
}
...
impl<'db> FailedJob<'db> {
    ...
    pub fn query(&self) -> FailedJobQuery<'db> {
        FailedJobQuery::new(self.db.clone(), self.base_url.clone())
    }
    pub fn insert(&self) -> FailedJobInsert<'db> {
        FailedJobInsert::new(self.db.clone(), self.base_url.clone())
    }
    pub fn update(&self) -> FailedJobUpdate<'db> {
        FailedJobUpdate::new(self.db.clone(), self.base_url.clone())
    }
    pub async fn find(&self, id: uuid::Uuid) -> Result<Option<FailedJobView>> {
...
impl<'db> FailedJobQuery<'db> {
    pub fn new(db: DbConn<'db>, base_url: Option<String>) -> Self {
        Self {
            ...
            base_url,
            select_sql: Some(
                "id, job_name, queue, payload, error, attempts, group_id, failed_at".to_string(),
            ),
            from_sql: None,
            count_sql: None,
            distinct: false,
    ...
    pub fn where_key_in<T: Clone + Into<BindValue>>(self, vals: &[T]) -> Self {
        self.where_in(FailedJobCol::Id, vals)
    }
    ...
    pub fn select_cols(mut self, cols: &[FailedJobCol]) -> Self {
        if cols.is_empty() {
            self.select_sql = Some(
                "id, job_name, queue, payload, error, attempts, group_id, failed_at".to_string(),
            );
        } else {
            let mut seen = std::collections::BTreeSet::new();
            let mut list: Vec<String> =
                "id, job_name, queue, payload, error, attempts, group_id, failed_at"
                    .split(',')
    ...
    pub fn add_select_cols(mut self, cols: &[FailedJobCol]) -> Self {
        ...
        let mut list: Vec<String> = match self.select_sql.take() {
            ...
            _ => "id, job_name, queue, payload, error, attempts, group_id, failed_at"
                .split(',')
                .map(|s| s.trim().to_string())
                .collect(),
        };
    ...
    fn add_select_raw(mut self, sql: impl Into<String>) -> Self {
        ...
        let mut base = self.select_sql.take().unwrap_or_else(|| {
            "id, job_name, queue, payload, error, attempts, group_id, failed_at".to_string()
        });
        if !base.is_empty() {
            base.push_str(", ");
    ...
    pub async fn get(self) -> Result<Vec<FailedJobView>> {
        let Self {
            db,
            base_url,
            select_sql,
    ...
    pub async fn first(self) -> Result<Option<FailedJobView>> {
        let mut v = self.limit(1).get().await?;
        Ok(v.pop())
    }

    pub async fn first_or_fail(self) -> Result<FailedJobView> {
        self.first()
            .await?
            .ok_or_else(|| anyhow::anyhow!("failed_jobs: record not found"))
    }

    pub async fn find(self, id: uuid::Uuid) -> Result<Option<FailedJobView>> {
        self.where_id(Op::Eq, id).first().await
    }
    pub async fn find_or_fail(self, id: uuid::Uuid) -> Result<FailedJobView> {
        self.find(id)
            .await?
            .ok_or_else(|| anyhow::anyhow!("failed_jobs: record not found"))
    }
    pub async fn first_or_create(
        self,
        create: impl FnOnce(FailedJobInsert<'db>) -> FailedJobInsert<'db>,
    ) -> Result<FailedJobView> {
        let db = self.db.clone();
        let base_url = self.base_url.clone();
        if let Some(existing) = self.first().await? {
            return Ok(existing);
        }
        let insert_builder = create(FailedJobInsert::new(db, base_url));
    ...
    pub async fn update_or_create(
        ...
        if let Some(existing) = self.first().await? {
            ...
            return FailedJob::new(db, base_url.clone())
                .query()
                .find(existing.id)
                .await
                .map(|r| r.unwrap());
        }
        let insert_builder = on_create(FailedJobInsert::new(db, base_url));
        insert_builder.save().await
    }
    ...
    pub async fn sole(self) -> Result<FailedJobView> {
    ...
    pub async fn pluck_pair<K, V>(
        self,
        extract: impl Fn(&FailedJobView) -> (K, V),
    ...
    pub async fn paginate(self, page: i64, per_page: i64) -> Result<Page<FailedJobView>> {
...
impl<'db> FailedJobUnsafeQuery<'db> {
    fn new(inner: FailedJobQuery<'db>) -> Self {
        Self { inner }
    }
    ...
    pub fn done(self) -> FailedJobQuery<'db> {
        self.inner
    }
}
...
impl<'db> FailedJobInsert<'db> {
    ...
    async fn save_with_db<'tx>(self, db: DbConn<'tx>) -> Result<FailedJobView> {
...
pub struct FailedJobUpdate<'db> {
    db: DbConn<'db>,
    base_url: Option<String>,
    sets: Vec<(FailedJobCol, BindValue)>,
    where_sql: Vec<String>,
    binds: Vec<BindValue>,
}
...
impl<'db> FailedJobUpdate<'db> {
    ...
    pub fn unsafe_sql(self) -> FailedJobUnsafeUpdate<'db> {
        FailedJobUnsafeUpdate::new(self)
    }
    pub fn set_id(mut self, val: uuid::Uuid) -> Self {
        self.sets.push((FailedJobCol::Id, val.into()));
        self
    }
    pub fn set_job_name(mut self, val: String) -> Self {
        self.sets.push((FailedJobCol::JobName, val.into()));
        self
    }
    pub fn set_queue(mut self, val: String) -> Self {
        self.sets.push((FailedJobCol::Queue, val.into()));
        self
    }
    pub fn set_payload(mut self, val: serde_json::Value) -> Self {
        self.sets.push((FailedJobCol::Payload, val.into()));
        self
    }
    pub fn set_error(mut self, val: String) -> Self {
        self.sets.push((FailedJobCol::Error, val.into()));
        self
    }
    pub fn set_attempts(mut self, val: i32) -> Self {
        self.sets.push((FailedJobCol::Attempts, val.into()));
        self
    }
    pub fn set_group_id(mut self, val: Option<String>) -> Self {
        self.sets.push((FailedJobCol::GroupId, val.into()));
        self
    }
    pub fn set_failed_at(mut self, val: time::OffsetDateTime) -> Self {
        self.sets.push((FailedJobCol::FailedAt, val.into()));
        self
    }
    pub fn where_id(mut self, op: Op, val: uuid::Uuid) -> Self {
        let idx = self.binds.len() + 1;
    ...
    async fn save_with_db<'tx>(self, db: DbConn<'tx>) -> Result<u64> {
        ...
        let select_sql = format!(
            "SELECT id FROM failed_jobs WHERE {}",
...
impl<'db> FailedJobUnsafeUpdate<'db> {
    fn new(inner: FailedJobUpdate<'db>) -> Self {
        Self { inner }
    }
    pub fn where_raw(mut self, clause: RawClause) -> Self {
        let (sql, binds) = clause.into_parts();
    ...
    pub fn done(self) -> FailedJobUpdate<'db> {
        self.inner
    }
}
...
impl GeneratedTableAdapter for FailedJobTableAdapter {
    ...
    fn apply_auto_filter<'db>(
        &self,
        query: FailedJobQuery<'db>,
        filter: &ParsedFilter,
        value: &str,
    ) -> anyhow::Result<Option<FailedJobQuery<'db>>>
    where
        Self: 'db,
    {
    ...
    fn apply_sort<'db>(
        ...
        column: &str,
        dir: SortDirection,
    ) -> anyhow::Result<FailedJobQuery<'db>>
    where
        Self: 'db,
    {
        let dir = match dir {
    ...
    fn cursor_from_row(&self, row: &FailedJobView, column: &str) -> Option<String> {
        match column {
    ...
    fn count<'db>(&self, query: FailedJobQuery<'db>) -> BoxFuture<'db, anyhow::Result<i64>>
    where
        Self: 'db,
    {
    ...
    fn fetch_page<'db>(
        ...
        page: i64,
        per_page: i64,
    ) -> BoxFuture<'db, anyhow::Result<Vec<FailedJobView>>>
    where
        Self: 'db,
    {
...
pub struct FailedJobDataTable<H = FailedJobDefaultDataTableHooks>
where
    H: FailedJobDataTableHooks,
{
    pub db: sqlx::PgPool,
    pub hooks: H,
...
impl<H: FailedJobDataTableHooks> AutoDataTable for FailedJobDataTable<H> {
    ...
    fn base_query<'db>(
        ...
        self.hooks
            .scope(FailedJob::new(&self.db, None).query(), input, ctx)
    }
