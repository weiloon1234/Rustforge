            ...
            if let Some(map) = self.translations.get("title") {
                ...
                if !filtered.is_empty() {
                    for id in &target_ids {
                        localized::upsert_localized_many(db.clone(), localized::CONTENT_PAGE_OWNER_TYPE, *id, "title", &filtered).await?;
                    }
                }
            }
            if let Some(map) = self.translations.get("content") {
                ...
                if !filtered.is_empty() {
                    for id in &target_ids {
                        localized::upsert_localized_many(db.clone(), localized::CONTENT_PAGE_OWNER_TYPE, *id, "content", &filtered).await?;
                    }
                }
            }
            if let Some(map) = self.translations.get("cover") {
                ...
                if !filtered.is_empty() {
                    for id in &target_ids {
                        localized::upsert_localized_many(db.clone(), localized::CONTENT_PAGE_OWNER_TYPE, *id, "cover", &filtered).await?;
                    }
                }
            }
        }
