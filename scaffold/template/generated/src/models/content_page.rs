...
pub trait ContentPageViewsExt {
    ...
    fn key_by<K>(
        &self,
        f: impl Fn(&ContentPageView) -> K,
    ) -> std::collections::HashMap<K, ContentPageView>
    where
        K: Eq + std::hash::Hash;
    fn group_by<K>(
        &self,
        f: impl Fn(&ContentPageView) -> K,
    ) -> std::collections::HashMap<K, Vec<ContentPageView>>
    where
        K: Eq + std::hash::Hash;
}
...
impl<'db> ContentPage<'db> {
    ...
    pub fn query(&self) -> ContentPageQuery<'db> {
        ContentPageQuery::new(self.db.clone(), self.base_url.clone())
    }
    pub fn insert(&self) -> ContentPageInsert<'db> {
        ContentPageInsert::new(self.db.clone(), self.base_url.clone())
    }
    pub fn update(&self) -> ContentPageUpdate<'db> {
        ContentPageUpdate::new(self.db.clone(), self.base_url.clone())
    }
...
impl<'db> ContentPageQuery<'db> {
    pub fn new(db: DbConn<'db>, base_url: Option<String>) -> Self {
        Self {
            db,
            base_url,
            select_sql: Some("id, tag, is_system, created_at, updated_at, deleted_at".to_string()),
            from_sql: None,
            count_sql: None,
            distinct: false,
            distinct_on: None,
    ...
    pub fn unsafe_sql(self) -> ContentPageUnsafeQuery<'db> {
        ContentPageUnsafeQuery::new(self)
    }
    pub fn where_id(mut self, op: Op, val: i64) -> Self {
        let idx = self.binds.len() + 1;
        self.where_sql.push(format!(
            "{} {} ${}",
            ContentPageCol::Id.as_sql(),
            op.as_sql(),
            idx
        ));
    ...
    pub fn where_is_system(mut self, op: Op, val: ContentPageSystemFlag) -> Self {
        ...
        self.where_sql.push(format!(
            "{} {} ${}",
            ContentPageCol::IsSystem.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_created_at(mut self, op: Op, val: time::OffsetDateTime) -> Self {
        ...
        self.where_sql.push(format!(
            "{} {} ${}",
            ContentPageCol::CreatedAt.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_updated_at(mut self, op: Op, val: time::OffsetDateTime) -> Self {
        ...
        self.where_sql.push(format!(
            "{} {} ${}",
            ContentPageCol::UpdatedAt.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_deleted_at(mut self, op: Op, val: Option<time::OffsetDateTime>) -> Self {
        ...
        self.where_sql.push(format!(
            "{} {} ${}",
            ContentPageCol::DeletedAt.as_sql(),
            op.as_sql(),
            idx
        ));
        self.binds.push(val.into());
    ...
    pub fn where_key_in<T: Clone + Into<BindValue>>(self, vals: &[T]) -> Self {
        self.where_in(ContentPageCol::Id, vals)
    }
    ...
    pub fn select_cols(mut self, cols: &[ContentPageCol]) -> Self {
        if cols.is_empty() {
            self.select_sql =
                Some("id, tag, is_system, created_at, updated_at, deleted_at".to_string());
        } else {
            let mut seen = std::collections::BTreeSet::new();
            let mut list: Vec<String> = "id, tag, is_system, created_at, updated_at, deleted_at"
                .split(',')
    ...
    pub fn add_select_cols(mut self, cols: &[ContentPageCol]) -> Self {
        ...
        let mut list: Vec<String> = match self.select_sql.take() {
            ...
            _ => "id, tag, is_system, created_at, updated_at, deleted_at"
                .split(',')
                .map(|s| s.trim().to_string())
                .collect(),
        };
    ...
    fn select_raw(mut self, sql: impl Into<String>) -> Self {
        ...
        if s.is_empty() {
            ...
            self.select_sql = Some(format!(
                "id, tag, is_system, created_at, updated_at, deleted_at, {}",
                s
            ));
        }
        self
    ...
    fn add_select_raw(mut self, sql: impl Into<String>) -> Self {
        ...
        let mut base = self.select_sql.take().unwrap_or_else(|| {
            "id, tag, is_system, created_at, updated_at, deleted_at".to_string()
        });
        if !base.is_empty() {
            base.push_str(", ");
        }
    ...
    pub async fn first_or_fail(self) -> Result<ContentPageView> {
        self.first()
            .await?
            .ok_or_else(|| anyhow::anyhow!("content_pages: record not found"))
    }
    ...
    pub async fn first_or_create(
        self,
        create: impl FnOnce(ContentPageInsert<'db>) -> ContentPageInsert<'db>,
    ...
    pub async fn update_or_create(
        ...
        if let Some(existing) = self.first().await? {
            ...
            return ContentPage::new(db, base_url.clone())
                .query()
                .find(existing.id)
                .await
                .map(|r| r.unwrap());
    ...
    pub async fn pluck_pair<K, V>(
        self,
        extract: impl Fn(&ContentPageView) -> (K, V),
    ...
    pub async fn restore(self) -> Result<u64> {
        if !HAS_SOFT_DELETE {
            anyhow::bail!("restore() not supported");
        }
        if self.limit.is_some() {
...
impl<'db> ContentPageUnsafeQuery<'db> {
    fn new(inner: ContentPageQuery<'db>) -> Self {
        Self { inner }
    }
    ...
    pub fn done(self) -> ContentPageQuery<'db> {
        self.inner
    }
}
...
impl<'db> ContentPageInsert<'db> {
    ...
    pub fn set_title_lang(mut self, locale: localized::Locale, val: impl Into<String>) -> Self {
        self.translations
            .entry("title")
            .or_default()
            .insert(locale.into(), val.into());
        self
    }
    pub fn set_title_langs(mut self, langs: localized::LocalizedText) -> Self {
        if !langs.en.is_empty() {
            self = self.set_title_lang(localized::Locale::En, langs.en);
        }
        if !langs.zh.is_empty() {
            self = self.set_title_lang(localized::Locale::Zh, langs.zh);
        }
        self
    }
    ...
    pub fn set_content_langs(mut self, langs: localized::LocalizedText) -> Self {
        if !langs.en.is_empty() {
            self = self.set_content_lang(localized::Locale::En, langs.en);
        }
        if !langs.zh.is_empty() {
            self = self.set_content_lang(localized::Locale::Zh, langs.zh);
        }
        self
    }
    ...
    pub fn set_cover_langs(mut self, langs: localized::LocalizedText) -> Self {
        if !langs.en.is_empty() {
            self = self.set_cover_lang(localized::Locale::En, langs.en);
        }
        if !langs.zh.is_empty() {
            self = self.set_cover_lang(localized::Locale::Zh, langs.zh);
        }
        self
    }
    ...
    async fn save_with_db<'tx>(self, db: DbConn<'tx>) -> Result<ContentPageView> {
        ...
        if !self.translations.is_empty() {
            ...
            if let Some(map) = self.translations.get("title") {
                ...
                for (loc, val) in map {
                    if supported.contains(&loc.as_str()) {
                        filtered.insert(loc.clone(), val.clone());
                    }
                }
                if !filtered.is_empty() {
                    localized::upsert_localized_many(
                        db.clone(),
                        localized::CONTENT_PAGE_OWNER_TYPE,
                        row.id,
                        "title",
                        &filtered,
                    )
...
impl<'db> ContentPageUpdate<'db> {
    ...
    pub fn unsafe_sql(self) -> ContentPageUnsafeUpdate<'db> {
        ContentPageUnsafeUpdate::new(self)
    }
    pub fn set_id(mut self, val: i64) -> Self {
    ...
    async fn save_with_db<'tx>(self, db: DbConn<'tx>) -> Result<u64> {
        ...
        let select_sql = format!(
            "SELECT id FROM content_pages WHERE {}",
...
impl<'db> ContentPageUnsafeUpdate<'db> {
    fn new(inner: ContentPageUpdate<'db>) -> Self {
        Self { inner }
    }
    pub fn where_raw(mut self, clause: RawClause) -> Self {
        let (sql, binds) = clause.into_parts();
    ...
    pub fn done(self) -> ContentPageUpdate<'db> {
        self.inner
    }
}
...
impl GeneratedTableAdapter for ContentPageTableAdapter {
    ...
    fn column_descriptors(&self) -> &'static [DataTableColumnDescriptor] {
        &[
            ...
            DataTableColumnDescriptor {
                name: "is_system",
                label: "Is System",
                data_type: "ContentPageSystemFlag",
                sortable: true,
                localized: false,
                filter_ops: &["eq", "gte", "lte"],
            },
            ...
            DataTableColumnDescriptor {
                ...
                sortable: false,
                localized: true,
                filter_ops: &["locale_eq", "locale_like"],
            },
            DataTableColumnDescriptor {
                name: "content",
                label: "Content",
        ...
        Self: 'db,
    {
        ...
        match filter {
            ...
            ParsedFilter::LocaleEq { column } => {
                let Some(field) = Self::parse_locale_field(column.as_str()) else {
                    return Ok(None);
                };
                ...
                Ok(Some(query.where_exists(
                    clause,
                    vec![
                        localized::CONTENT_PAGE_OWNER_TYPE.to_string(),
                        field.to_string(),
                        locale,
                        trimmed.to_string(),
                    ],
    ...
    fn apply_sort<'db>(
        ...
        column: &str,
        dir: SortDirection,
    ) -> anyhow::Result<ContentPageQuery<'db>>
    where
        Self: 'db,
    {
        let dir = match dir {
    ...
    fn cursor_from_row(&self, row: &ContentPageView, column: &str) -> Option<String> {
        match column {
            ...
            "updated_at" => row
                ...
                .format(&time::format_description::well_known::Rfc3339)
                .ok(),
            "deleted_at" => row.deleted_at.as_ref().and_then(|v| {
    ...
    fn count<'db>(&self, query: ContentPageQuery<'db>) -> BoxFuture<'db, anyhow::Result<i64>>
    where
        Self: 'db,
    {
    ...
    fn fetch_page<'db>(
        ...
        page: i64,
        per_page: i64,
    ) -> BoxFuture<'db, anyhow::Result<Vec<ContentPageView>>>
    where
        Self: 'db,
    {
...
pub struct ContentPageDataTable<H = ContentPageDefaultDataTableHooks>
where
    H: ContentPageDataTableHooks,
{
    pub db: sqlx::PgPool,
    pub hooks: H,
...
impl<H: ContentPageDataTableHooks> AutoDataTable for ContentPageDataTable<H> {
    ...
    fn base_query<'db>(
        ...
        self.hooks
            .scope(ContentPage::new(&self.db, None).query(), input, ctx)
    }
