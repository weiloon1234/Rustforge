...
fn render_model(
    ...
    if emit_hooks {
        ...
        writeln!(
            out,
            "                        let event = ModelEvent {{ model: \"{model_snake}\", table: \"{table}\", record_key: Some(format!(\"{{}}\", old_row.{pk_snake})) }};"
        )
        .unwrap();
        writeln!(
            out,
