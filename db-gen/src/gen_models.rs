...
fn render_patch_model_impl(
    ...
    if !touch_targets.is_empty() {
        writeln!(
            out,
            "            let ids: Vec<i64> = target_ids.iter().map(|v| *v as i64).collect();"
        )
        .unwrap();
        writeln!(
            out,
            "            core_db::common::model_api::persist_patch_touch_targets(db.clone(), \"{table}\", \"{pk}\", &ids, Self::touch_targets()).await?;"
        )
        .unwrap();
    }
    if !localized_fields.is_empty() {
...
fn render_feature_persistence_model_impl(
    ...
    if !touch_targets.is_empty() {
        // Generate touch_targets() returning static descriptor array
        writeln!(
            out,
            "    fn touch_targets() -> &'static [core_db::common::model_api::TouchTarget] {{"
        )
        .unwrap();
        writeln!(out, "        &[").unwrap();
        ...
        // Generate extract_touch_fk_values() reading FK fields from the row
        writeln!(
            out,
            "    fn extract_touch_fk_values(row: &<Self as core_db::common::model_api::RuntimeModel>::Row) -> Vec<(&'static str, Option<i64>)> {{"
        )
        .unwrap();
        ...
        // Generate persist_create_related delegating to runtime function
        writeln!(
            out,
        ...
        .unwrap();
        writeln!(out, "        let fk_values = Self::extract_touch_fk_values(&row);").unwrap();
        writeln!(out, "        Box::pin(async move {{").unwrap();
        writeln!(out, "            core_db::common::model_api::persist_create_touch_targets(db, &fk_values, Self::touch_targets()).await").unwrap();
        writeln!(out, "        }})").unwrap();
        writeln!(out, "    }}").unwrap();

        // Generate persist_patch_related delegating to runtime function
        writeln!(
            out,
        ...
        writeln!(
            out,
            "            let ids: Vec<i64> = target_ids.into_iter().map(|v| v as i64).collect();"
        )
        .unwrap();
        writeln!(
            out,
            "            core_db::common::model_api::persist_patch_touch_targets(db, \"{table}\", \"{pk}\", &ids, Self::touch_targets()).await"
        )
        .unwrap();
        writeln!(out, "        }})").unwrap();
        writeln!(out, "    }}").unwrap();
...
struct ModelCtx<'a> {
    ...
    record_ident: String,
    col_ident: String,
    public_col_ident: String,
    base_select: String,
    fields: Vec<FieldSpec>,
    db_fields: Vec<FieldSpec>,
    localized_fields: Vec<String>,
    enum_explained_fields: Vec<EnumExplainedFieldSpec>,
    enum_specs: BTreeMap<String, EnumSpec>,
...
fn render_imports_and_constants(ctx: &ModelCtx) -> (String, String) {
    ...
    if ctx.use_snowflake_id {
        writeln!(imports, "use core_db::common::sql::{{BindValue, Op, OrderDir, SetMode, bind, bind_query, bind_scalar, generate_snowflake_i64, is_sql_profiler_enabled, format_duration, record_profiled_query, DbConn}};").unwrap();
    } else {
        writeln!(imports, "use core_db::common::sql::{{BindValue, Op, OrderDir, SetMode, bind, bind_query, bind_scalar, is_sql_profiler_enabled, format_duration, record_profiled_query, DbConn}};").unwrap();
    }
    ...
    writeln!(imports, "use core_db::platform::localized::types::LocalizedMap;").unwrap();
    writeln!(imports, "use crate::generated::models::common::{{FieldChange, FieldInput, Page, log_observer_error, renumber_placeholders}};").unwrap();
    writeln!(imports, "use core_db::common::model_api::{{ColExpr, Column, Create, CreateState, ManyRelation, ModelDef, OneRelation, Patch, PatchState, Query, QueryState}};").unwrap();
    if ctx.has_meta {
        writeln!(imports, "use core_db::platform::meta::types::MetaMap;").unwrap();
    }
    if !ctx.localized_fields.is_empty() || ctx.has_meta || ctx.has_attachments {
        writeln!(imports, "use crate::generated::localized;").unwrap();
    }
    if !ctx.localized_fields.is_empty() {
        writeln!(imports, "use core_i18n::current_locale;").unwrap();
    }
    {
        let mut imported_models = std::collections::BTreeSet::new();
        // Import model types for direct relations and nested relation paths (datatable filters)
        let rel_targets = ctx.relations.iter().map(|r| (&r.target_model, &ctx.model_snake));
        let path_targets = ctx.relation_paths.iter().map(|p| (&p.target_model, &ctx.model_snake));
        for (target_model, model_snake) in rel_targets.chain(path_targets) {
            let target_mod = to_snake(target_model);
            if target_mod != *model_snake && imported_models.insert(target_mod.clone()) {
                let target_title = to_title_case(target_model);
                let target_relations = ctx.schema.models.get(target_model)
                    .map(|model| parse_relations(ctx.schema, model, target_model, &[]))
                    .unwrap_or_default();
    ...
    if !ctx.localized_fields.is_empty() {
        writeln!(imports, "use crate::generated::localized::LocalizedMapHelper;").unwrap();
    }
    if !ctx.enum_explained_fields.is_empty() {
        writeln!(imports, "use super::enums::*;").unwrap();
    }
    if ctx.emit_hooks {
        writeln!(imports, "use core_db::common::model_observer::{{ModelEvent, ObserverAction, try_get_observer}};").unwrap();
    }

    let mut constants = String::new();
    writeln!(constants, "const HAS_CREATED_AT: bool = {};", ctx.has_created_at).unwrap();
    writeln!(constants, "const HAS_UPDATED_AT: bool = {};", ctx.has_updated_at).unwrap();
    writeln!(constants, "const HAS_SOFT_DELETE: bool = {};", ctx.has_soft_delete).unwrap();
    (imports, constants)
}

fn render_active_record_section(ctx: &ModelCtx) -> String {
    let model_title = &ctx.model_title;
    let parent_pk_ty = &ctx.parent_pk_ty;
    let mut out = String::new();
    writeln!(out).unwrap();
    ...
    writeln!(out, "    async fn find(db: &sqlx::PgPool, id: Self::Id) -> anyhow::Result<Option<Self>> {{").unwrap();
    writeln!(out, "        {model_title}Model::find(db, id).await.map_err(|e| e.into())").unwrap();
    writeln!(out, "    }}").unwrap();
    writeln!(out, "}}").unwrap();
    out
}
...
fn render_column_model_section(ctx: &ModelCtx) -> String {
    let model_snake = &ctx.model_snake;
    let model_title = &ctx.model_title;
    let row_ident = &ctx.row_ident;
    let record_ident = &ctx.record_ident;
    let col_ident = &ctx.col_ident;
    let pk = &ctx.pk;
    let parent_pk_ty = &ctx.parent_pk_ty;
    let fields = &ctx.fields;
    let db_fields = &ctx.db_fields;
...
fn render_row_view_json_section(ctx: &ModelCtx) -> String {
    ...
    let hidden_fields: &BTreeSet<String> = &ctx.hidden_fields;
    let meta_fields: &[crate::schema::MetaFieldSpec] = &ctx.meta_fields;
    let single_attachments: &[AttachmentFieldSpec] = &ctx.single_attachments;
    let multi_attachments: &[AttachmentFieldSpec] = &ctx.multi_attachments;
...
fn render_model_runtime_section(ctx: &ModelCtx) -> String {
    ...
    let has_meta = ctx.has_meta;
    let has_attachments = ctx.has_attachments;
    let has_soft_delete = ctx.has_soft_delete;
    let has_created_at = ctx.has_created_at;
    let has_updated_at = ctx.has_updated_at;
    let use_snowflake_id = ctx.use_snowflake_id;
...
writeln!(out, "}}\n").unwrap();
if !relations.is_empty() {
    let model_query_ty = "QueryState<'db>";
    for (rel_idx, rel) in relations.iter().enumerate() {
        let rel_name = to_snake(&rel.name);
        let target_table = &rel.target_table;
        let target_pk = &rel.target_pk;
        ...
        match rel.kind {
            RelationKind::BelongsTo | RelationKind::HasOne => {
                ...
                .unwrap();
                writeln!(out, "        record.{rel_name}.as_deref()").unwrap();
                writeln!(out, "    }}").unwrap();
                writeln!(out, "}}\n").unwrap();
            }
            RelationKind::HasMany => {
                ...
                .unwrap();
                writeln!(out, "        &record.{rel_name}").unwrap();
                writeln!(out, "    }}").unwrap();
                writeln!(out, "}}\n").unwrap();
...
fn render_datatable_section(ctx: &ModelCtx) -> String {
    ...
    let localized_fields: &[String] = &ctx.localized_fields;
    let enum_explained_fields: &[EnumExplainedFieldSpec] = &ctx.enum_explained_fields;
    let computed_fields: &[crate::schema::ComputedFieldSpec] = &ctx.computed_fields;
    let hidden_fields: &BTreeSet<String> = &ctx.hidden_fields;
    let single_attachments: &[AttachmentFieldSpec] = &ctx.single_attachments;
    let multi_attachments: &[AttachmentFieldSpec] = &ctx.multi_attachments;
    let has_meta = ctx.has_meta;
    let use_snowflake_id = ctx.use_snowflake_id;
    let relation_paths: &[RelationPathSpec] = &ctx.relation_paths;
    let schema = ctx.schema;
...
if options.include_datatable {
    ...
    let unique_targets: BTreeMap<String, Vec<&RelationPathSpec>> = {
        let mut map = BTreeMap::new();
        for rel_path in relation_paths {
            map.entry(rel_path.target_model.clone()).or_insert_with(Vec::new).push(rel_path);
        }
        map
    };
...
.unwrap();
    ...
    // ── DataTableColumnResolver impl ────────────────────────────────
    writeln!(
        out,
        "impl DataTableColumnResolver for {table_adapter_ident} {{"
    )
    .unwrap();

    // resolve_col_sql
    writeln!(out, "    fn resolve_col_sql(&self, column: &str) -> Option<&'static str> {{").unwrap();
    writeln!(out, "        match column {{").unwrap();
    for f in db_fields {
        writeln!(out, "            \"{name}\" => Some(\"{name}\"),", name = f.name).unwrap();
    }
    writeln!(out, "            _ => None,").unwrap();
    writeln!(out, "        }}").unwrap();
    writeln!(out, "    }}").unwrap();

    // resolve_like_col_sql
    writeln!(out, "    fn resolve_like_col_sql(&self, column: &str) -> Option<&'static str> {{").unwrap();
    writeln!(out, "        match column {{").unwrap();
    for f in db_fields {
        if f.ty.contains("String") {
            writeln!(out, "            \"{name}\" => Some(\"{name}\"),", name = f.name).unwrap();
    ...
    // parse_bind_for_col (delegates to the existing static method)
    writeln!(out, "    fn parse_bind_for_col(&self, column: &str, raw: &str) -> Option<BindValue> {{").unwrap();
    writeln!(out, "        Self::parse_bind_for_col(column, raw)").unwrap();
    writeln!(out, "    }}").unwrap();

    // resolve_locale_field
    writeln!(out, "    fn resolve_locale_field(&self, column: &str) -> Option<&'static str> {{").unwrap();
    writeln!(out, "        Self::parse_locale_field(column)").unwrap();
    writeln!(out, "    }}").unwrap();

    // parse_datetime
    writeln!(out, "    fn parse_datetime(&self, raw: &str, end_of_day: bool) -> Option<time::OffsetDateTime> {{").unwrap();
    writeln!(out, "        Self::parse_datetime(raw, end_of_day)").unwrap();
    writeln!(out, "    }}").unwrap();

    // table_name
    writeln!(out, "    fn table_name(&self) -> &'static str {{ \"{table}\" }}").unwrap();

    // pk_column
    writeln!(out, "    fn pk_column(&self) -> &'static str {{ \"{pk}\" }}").unwrap();

    // locale_owner_type
    if !localized_fields.is_empty() {
        writeln!(out, "    fn locale_owner_type(&self) -> Option<&'static str> {{ Some(localized::{model_snake_upper}_OWNER_TYPE) }}").unwrap();
    }
...
.unwrap();
    // Delegate standard (non-relation) filters to core-datatable shared dispatch
    writeln!(out, "        if let Some(state) = core_datatable::apply_standard_filter(query.clone().into_inner(), filter, value, self)? {{").unwrap();
    writeln!(out, "            return Ok(Some(Query::from_inner(state)));").unwrap();
    writeln!(out, "        }}").unwrap();
    ...
    .unwrap();
    // Relation filters remain per-model (require typed where_has)
...
.unwrap();
    ...
    writeln!(out, "                }}").unwrap();
    writeln!(out, "            }}").unwrap();
    // Standard filters already handled above via apply_standard_filter
    writeln!(out, "            _ => Ok(None),").unwrap();
    writeln!(out, "        }}").unwrap();
    writeln!(out, "    }}").unwrap();
...
fn render_model(
    ...
    let ctx = ModelCtx::new(name, cfg, schema, cfgs, options);

    let (imports, constants) = render_imports_and_constants(&ctx);
    let row_view_json_section = render_row_view_json_section(&ctx);
    let column_model_section = render_column_model_section(&ctx);
    let datatable_section = render_datatable_section(&ctx);
    let active_record_section = render_active_record_section(&ctx);
    let model_runtime_section = render_model_runtime_section(&ctx);

    let mut context = TemplateContext::new();
    context.insert("imports", imports.trim_end().to_string()).unwrap();
    context.insert("constants", constants.trim_end().to_string()).unwrap();
    context.insert("row_view_json_section", row_view_json_section.trim_start().to_string()).unwrap();
    context.insert("column_model_section", column_model_section.trim_start().to_string()).unwrap();
    context.insert("datatable_section", datatable_section.trim_start().to_string()).unwrap();
    context.insert("active_record_section", active_record_section.trim_start().to_string()).unwrap();
    context.insert("model_runtime_section", model_runtime_section.trim_start().to_string()).unwrap();
