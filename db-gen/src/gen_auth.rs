...
pub fn generate_auth(
    ...
    for (name, config) in &configs.auth.guards {
        ...
        let provider_cfg = schema.models.get(provider).unwrap_or_else(|| {
            panic!(
        ...
        writeln!(f, "use core_db::common::sql::{{DbConn, Op}};")?;
        ...
        match provider_pk_ty.as_str() {
            "uuid::Uuid" => {
                ...
                writeln!(
                    f,
                    "        {}Query::new(db, None).find(parsed).await",
                    provider_title
                )?;
            }
            "i16" | "i32" | "i64" | "u16" | "u32" | "u64" | "isize" | "usize" => {
                writeln!(
                    f,
                    "        let parsed = match id.trim().parse::<{}>() {{ Ok(v) => v, Err(_) => return Ok(None), }};",
                    provider_pk_ty
                )?;
                writeln!(
                    f,
            ...
            "String" => {
                writeln!(
                    f,
                    "        {}Query::new(db, None).find(id.to_string()).await",
                    provider_title
                )?;
            }
            other => {
