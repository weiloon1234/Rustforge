...
pub fn validate_attachment(
    ...
    if !ok {
        anyhow::bail!(
            "Attachment type not allowed (allowed: {:?}, got: {} {:?})",
            rules.allowed,
            content_type,
            filename
        );
...
pub fn maybe_resize(
    ...
    data: Bytes,
    content_type: &str,
) -> anyhow::Result<(Bytes, Option<(i32, i32)>, String)> {
    ...
    let (target_w, target_h) = match (resize.width, resize.height) {
        (Some(w), Some(h)) => (w, h),
        (Some(w), None) => {
            let h = ((orig_h as f32) * (w as f32 / orig_w as f32))
                .round()
                .max(1.0) as u32;
            (w, h)
        }
        (None, Some(h)) => {
            let w = ((orig_w as f32) * (h as f32 / orig_h as f32))
                .round()
                .max(1.0) as u32;
            (w, h)
        }
        (None, None) => {
            return Ok((
                data,
                Some((orig_w as i32, orig_h as i32)),
    ...
    if ct_lower.contains("jpeg") || ct_lower.contains("jpg") {
        ...
        Ok((
            Bytes::from(buf),
            Some((resized.width() as i32, resized.height() as i32)),
