...
use tokens::phonenumber::phonenumber_tokens;
...
impl ToTokens for ValidateField {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        ...
        // Phone number by country field validation
        let phonenumber = if let Some(phonenumber) = self.phonenumber.clone() {
            wrapper_closure(phonenumber_tokens(
                &self.crate_name,
                phonenumber,
                &actual_field,
                &field_name_str,
