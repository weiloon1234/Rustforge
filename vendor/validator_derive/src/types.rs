...
impl ValidateField {
    pub fn validate(&self, struct_ident: &Ident, all_fields: &[&Field], current_field: &Field) {
        ...
        if let Some(phonenumber) = &self.phonenumber {
            let other = match phonenumber.field.get_ident() {
                Some(field) => field.clone(),
                None => {
                    abort!(
                        phonenumber.field.span(),
                        "Invalid attribute for #[validate(phonenumber(...))] on field `{}`:",
                        field_name;
                        note = "The `field` value must be a direct field identifier";
                        help = "Use `field = contact_country_iso2`"
                    )
                }
            };

            if !all_fields.iter().any(|f| f.ident.as_ref() == Some(&other)) {
                abort!(
                    phonenumber.field.span(), "Invalid attribute for #[validate(phonenumber(...))] on field `{}`:", field_name;
                    note =  "The `field` doesn't exist in the struct `{}`", struct_ident;
                    help = "Add the field `{}` to the struct", other
                )
            }
        }
